...
> 范围外：见 [`后续待完成.md`](./后续待完成.md)（开实现前必读；尤其勿提前做六时/雷劫/宗门/WS）  
> **灵宠完整细则**：[`灵宠系统设计.md`](./灵宠系统设计.md)（N4 收窄出口仍见本文 §7.4）  
> 版本：v1.0 · 2026-08-05（§7.4 于 2026-08-07 收窄；**§6.4 傀儡神识** 2026-08-17）  
...
| 项 | 对齐 |
| --- | --- |
| 领域对象 | `PlayerCharacter` · `AvatarCharacter` · `PetCharacter` · `PuppetCharacter`/`PuppetItem` · 工坊 Content/Item |
| 配置 | 经 ContentStore（avatar/pets/craft…）；测试可 `yaml_authority` |
| 战斗 | 双线程棋子经 `on_battle_enter` → BattleUnitSeed 进 Bench/快照；引擎纯度不变 |
| 增量 | 灵宠技能等可 Ability-izable；禁止按 id 写死分支 |
...
| **延后必登记** | ~~多化身~~（**已取消**→[`化身系统设计.md`](./化身系统设计.md)）；~~化身独立跨境~~（**v1.3 落入 AVATAR-D08**）；道值炼丹、M3 引擎打磨等写入 `后续待完成.md` |
...
| **M4-D02** | 化身独立跨大境界突破权 | **落入** [`化身系统设计.md`](./化身系统设计.md) **v1.3 / AVATAR-D08** |
...
| 境界展示 | 化身自有境界字段（v1.3）；相对本体封顶 + 禁道主/轮回境；实现 **AVATAR-D08** |
...
| 占用 | **装备栏**结算：化身开关开 × `cost_avatar`；灵宠槽 × `cost_pet`（物种可覆盖）；傀儡编成 × `cost_puppet`（试炼木傀不计）。**与棋盘是否落子无关**。三者累加进同一神识池 |
...
### 6.4 傀儡与神识（编成最多 3 只 · 战力看神识）

> **玩法**：傀儡 **不能**用装备单槽冒充编成；编成板上阵 **最多 3 只**（`divine_sense.puppet_loadout_max`）。**禁止**因神识不够拒绝上阵——超量只削弱。灵宠仍 **只能 1 只**。角色页编成见 [`道具与装备实体设计.md`](./道具与装备实体设计.md) **§5.3**。  
> 共享神识池（§6.1）**含**化身 + 灵宠 + 上阵傀儡；角色简示栏分子=已用合计，分母=自身总神识。编成板仍可按傀儡自身占用预览发挥%。

| 项 | 约定 |
| --- | --- |
| 件数 | 背包可多只；编成板 **最多 3 只**；棋盘落子仍受 `max_units` + 部署格 |
| 硬拒 | 第 4 只勾选/保存拒绝；**禁止**因神识不足拒绝已勾选的 1～3 只 |
| 计费对象 | 角色面板：编成板已上阵真傀 + 进攻预设化身/灵宠；开战：棋盘 `unit_kind` 为 avatar/pet/puppet 且非试炼木傀 |
| 占用 | 并入 §6.1 共享池；默认 `divine_sense.costs.puppet`；分母 = 角色神识容量 |
| 乘区 | 总占用套同一张 `overload_bands`；编成板勾选预览可用傀儡自身 load |
| 作用面 | 开战超载乘区打进攻方棋子（与化身/灵宠同一套 `snapshot_for_character`） |
| 舒适区 | 合计 load ≤ 角色神识容量 → 满发挥 |
| 反噬 | 合计 load > hard_cap 才写 `divine_sense_backlash` |
| 试炼木傀 | 不计入占用，不套本乘区 |

```yaml
# divine_sense.yaml 增量（ITEM-0 或紧随实现；缺省视为 0=尚未打开乘区）
costs:
  avatar: 5
  pet: 3
  puppet: 2    # 每只上场真傀；与化身/灵宠合计进同一池
```

`DIVINE_SENSE_STRICT=false` 时开战只打日志不衰减（与共享池一致）。
...
| --- | --- |
| 持有上限 | 配置（如 5）；超额拒绝 |
| 上阵 | 占棋子位；**硬顶 1**（穿戴栏 `pet` 格 + `unit_kinds.pet.unique=true`）；神识只衰减这一只的强度，不再用「多宠解锁」 |
...
| 2026-08-18 | **神识池合并**：上阵化身+灵宠+傀儡累加同一池；简示栏分子=已用、分母=总神识 |
| 2026-08-17 | **傀儡神识 §6.4**：件数无槽位硬顶；`load_puppet` 平行乘区只打傀儡棋子；灵宠上阵硬顶 1；与道具专文 v0.7 对齐 |
