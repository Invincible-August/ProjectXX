...
> 范围外：见 [`后续待完成.md`](./后续待完成.md)（开设计前必读）  
> 版本：v1.0 · 2026-08-03
...
| 项 | 对齐 |
| --- | --- |
| 领域对象 | `PlayerCharacter`：挂机 settle / 突破 / 教学 PVE 门面 |
| 配置 | 经 ContentStore（idle/breakthrough/battle 域）；测试可 `yaml_authority` 只改 YAML |
| 战斗 | `on_battle_enter` → BattleUnitSeed；教学战仍可走现行路径，纯度目标对齐 `simulate_battle` |
