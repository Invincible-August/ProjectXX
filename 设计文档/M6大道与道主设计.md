...
> 范围外：见 [`后续待完成.md`](./后续待完成.md)（开实现前必读；尤其勿提前做多频道聊天/传承红包/宗门战成型/完整 37 道数值）  
> 版本：v1.1 · 2026-08-10  
...
| 项 | 对齐 |
| --- | --- |
| 领域对象 | `LawEntity`（大道/道池/道主）· `PlayerCharacter`；WS 为平台横切非 Character 子类 |
| 配置 | 经 ContentStore（dao / dao_lord…）；测试可 `yaml_authority` |
| 战斗 | 道运用进战报仍经开战门面 → BattleUnitSeed；`simulate_battle` 纯度不变 |
| 增量 | 道技/克制长期 Ability/op；禁止按道 id 写死引擎分支 |
