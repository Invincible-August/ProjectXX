...
> 范围外：见 [`后续待完成.md`](./后续待完成.md)（开实现前必读；尤其勿提前做棋盘 / 雷劫 / WS）  
> 版本：v1.0 · 2026-08-04  
...
| 项 | 对齐 |
| --- | --- |
| 领域对象 | `PlayerCharacter` + Content（体质/功法/品阶）；三向挂机与分配门面 |
| 配置 | 经 ContentStore；测试可 `CONTENT_STORE_MODE=yaml_authority` 只改 YAML |
| 战斗 | 养成组装 ATTR → 开战仍 `on_battle_enter` → BattleUnitSeed |
...
6. 体质：初始 1 本源 + 2 旁支；轮回点可购更多旁支（软顶 7，无硬顶）；同一体质本源/旁支两套效果；收藏区不进道具背包；格满拒绝；升品/融合接口可调（结果可占位）。  
...
slot_defaults:
  main: 1                 # 本源
  sub: 2                  # 初始旁支；更多用轮回点买
  soft_cap: 7             # 软上限（提示，非硬拒）
items:
  sample_body_root:
    name: 凡体·根基
    quality: mortal
    base_attrs: { vitality: 5, defense: 3 }
    main_effects: { hp_bonus: 8 }
    sub_effects: { atk_bonus: 2 }
  sample_main_affix_iron:
    name: 铜皮凡体
    main_effects: { hp_bonus: 20 }
    sub_effects: { atk_bonus: 4 }
  sample_sub_affix_swift:
    name: 轻身凡体
    main_effects: { atk_bonus: 6 }
    sub_effects: { atk_bonus: 2 }
```
...
| 表 | 用途 |
| --- | --- |
| `character_constitution_slots` | 角色本源/旁支数量与已镶嵌 `item_instance_id` |
| `character_constitution_items` | 收藏实例（独立于道具背包）：`def_id`、品质/品级、是否已镶嵌 |
| （可选）`constitution_ops_log` | 升品/融合审计 |

创角发放：根基 + 铜皮 + 两件轻身（收藏区，便于同名冲突联调）。同一体质装本源与旁支效果不同。
...
| 日期 | 说明 |
| --- | --- |
| 2026-08-18 | 体质：1 本源 + N 旁支；轮回点购槽软顶 7；同一体质两套效果；收藏区不进背包 |
| 2026-08-14 | ARCH 对齐节 |
