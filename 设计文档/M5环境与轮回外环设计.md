...
> 范围外：见 [`后续待完成.md`](./后续待完成.md)（开实现前必读；尤其勿提前做道主/完整引渡社交/地图野外捕获/真 WS 基建）  
...
| 项 | 对齐 |
| --- | --- |
| 领域对象 | `Environment`（六时/天气）· `PlayerCharacter` 状态机（渡劫/轮回）· Content 雷劫/轮回表 |
| 配置 | 经 ContentStore（calendar/weather/tribulation/reincarnation…）；测试可 `yaml_authority` |
| 战斗 | 开战/渡劫锁定环境快照；单位仍 `on_battle_enter` → BattleUnitSeed |
| 增量 | 环境/雷劫修正优先配置与 Ability/乘区；禁止按 id 写死分支 |
...
| **M5-D07** | ~~化身独立跨境/化身渡劫~~ | **取消**（v1.3：不渡劫；突破见 AVATAR-D08） |
...
| 类别 | M5 落地（强化后） |
| --- | --- |
| 体质与已镶嵌 | **实例全保留**（独立收藏区，不进道具背包）；栏位 = 1 本源 + N 旁支（`slots.constitution` 初始 3 + 次数免费 + 商店购，**软顶 7 无硬顶**）；超额卸下仅在槽数回落时 |
| 境界与修为池 | **重置**锻体一层；三池清零 |
| 化身 | **解散** |
...
| 2026-08-18 | 体质槽：1 本源 + N 旁支；轮回点购槽软顶 7、无硬拒；同一体质本源/旁支两套效果 |
