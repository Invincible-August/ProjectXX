...
| 项 | 对齐 |
| --- | --- |
| 领域对象 | **`PetCharacter`** + Item **`pet_egg`**；物种/词条/技能为 Content；技能后期 **Ability-izable** |
| 配置 | 经 ContentStore（pets 域热插拔）；测试可 `yaml_authority` 只改 YAML |
| 战斗 | 自走棋：`PetCharacter.on_battle_enter` → BattleUnitSeed；回合制对战独立、不复用棋盘引擎 |
| 增量 | 新宠/新招只加数据；四技能长期可 Ability grants；禁止按 species_id 写死分支 |
...
- **神识**：上阵化身 + **那一只**灵宠 + 上阵傀儡占同一神识池；品阶高 ≠ 无视神识。完整公式见 M4-D03。
- **自走棋上阵硬顶 1**：与 [`道具与装备实体设计.md`](./道具与装备实体设计.md) §5.3 对齐；包内可多养，出场只能 1 只。
...
| 日期 | 说明 |
| --- | --- |
| 2026-08-18 | **v2.2.1** §1.3：共享神识池含上阵傀儡 |
| 2026-08-14 | ARCH 对齐节 |
