...
> 范围外：见 [`后续待完成.md`](./后续待完成.md)（开实现前必读；尤其勿提前做地图/势力频、战场社交、化身双修、真支付、ATTR 满表）  
> 版本：v1.0 · 2026-08-11  
...
| 项 | 对齐 |
| --- | --- |
| 领域对象 | `Organization`（宗门）· `PlayerCharacter` / `AvatarCharacter`（任务/助战）；交易/邮件为服务层 |
| 配置 | 经 ContentStore（sects/trade/chat…）；测试可 `yaml_authority` |
| 战斗 | 助战/赛会棋子仍 `on_battle_enter` → BattleUnitSeed；引擎纯度不变 |
...
| D26 | **角色 Presence** | WS 鉴权在线 + grace；组队/面交/助战/道友/赛会共用门面；细则 [`玩家在线状态设计.md`](./玩家在线状态设计.md) | **横切** |
...
等级键（低→高）：`hut` 草庐 → `mountain_gate` 山门 → `spirit_sect` 灵宗 → `famous_sect` 名宗 → `superior_sect` 上宗 → `immortal_gate` 仙门 → `sacred_land` 圣地 → `heaven_sect` 天宗 → `dao_court` 道庭。

| 字段 | 说明 |
| --- | --- |
| `max_members` | 可招募修士上限 |
| `facility_level_cap` | 设施等级上限 |
| `upgrade_require_facilities` | 升到本档前设施须达等级 |
| `max_active_buffs` / `buff_tier_cap` | 同时开启增益数 / 档位上限 |
| `idle_bonus` | 入宗挂机占位乘区 |

- 玩家自建：从草庐起，可升等级；扣宗门灵石库。  
- NPC：模板写死 `grade`/`specialty`，**不可**升宗门等级。  
- API：`GET /sect/overview`、`POST /sect/grade/upgrade`、`POST /sect/facilities/{id}/upgrade`、`POST /sect/buffs/toggle`。

### 3.8 弟子职位与任命（M7-V+）

十二档：`laborer` 杂役 → … → `founder` 创派祖师。  
掌门 / 创派祖师 **唯一**；太上由创派任命；大长老由太上/创派任命且修为须高于掌门；外门长老及之前可贡献自申（次日游戏日自动通过）；内门执事/内门长老等可毛遂自荐后由有权者任命；任命后 **当日不可再改**。

议事厅权限矩阵见 `sects.yaml` `disciple_ranks.*.council_actions`（俸禄/自荐/任命/公告/升设施/升等级/buff/战入口）。

API：`GET /sect/members`、`POST /sect/ranks/apply`、`POST /sect/ranks/appoint`、`POST /sect/council/salary`、`POST /sect/council/announce`；`POST /sect/council/war/start` → **501 / 40110 占位（真结算 → M11）**。

### 3.9 十设施（M7-V+ 可玩骨架）

| 设施键 | 中文 | 本轮能力 |
| --- | --- | --- |
| `quest_hall` | 任务殿 | 复用既有 `/sect/quests` |
| `council_hall` | 议事厅 | 人事/公告/俸禄/战占位 |
| `treasure_pavilion` | 藏宝阁 | 贡献兑换；按职位页权限放入/分配；禁图纸类型 |
| `scripture_pavilion` | 藏经阁 | 兑换/上供；专精匹配加贡献；自研须审核 |
| `forge_workshop` / `alchemy_workshop` / `talisman_workshop` | 锻造工坊 / 炼丹阁 / 服务工坊 | 贡献聘工匠；上缴与兑换图纸；仅收录图纸可代工 |
| `formation_array` | 宗门大阵 | 无管理权：兑换/上缴阵法；有权：选阵、启停、属性加点（攻/防/抗） |
| `spirit_mine` | 矿脉 | 产出持续入宗门灵石库；采矿挂机占名额、耗体力得个人灵石并加速入库 |
| `herb_garden` | 灵药园 | 直接兑换灵植；自行/托管种植（托管须灵植师）；职位/设施影响地块数 |

建宗必选 `specialty`；初始各设施 Lv1。配置后台域 `sects` 已扩字段中文 schema。

**本轮不做**：真势力战/宗门战结算（M11）；inventory 图纸 `item_type` 真背包键（仅登记禁止类型）；工坊真扣材料入包（占位贡献闭环）。

### 3.10 配置示意 `sects.yaml`（摘录）

```yaml
create_cost_spirit_stones: 100000
sect_grades:
  hut: { label_zh: 草庐, max_members: 20, facility_level_cap: 2 }
disciple_ranks:
  founder: { label_zh: 创派祖师, unique: true, treasury_page_max: 99 }
specialties:
  sword: { label_zh: 剑修 }
npc_sects:
  lingshou_zong:
    ...
    specialty: beast
facility_defs:
  council_hall: { label_zh: 议事厅, initial_level: 1 }
```

> 完整表见 `backend/app/config_data/sects.yaml`；旧「祖师境界功能 / 兑宠」仍保留。
...
- [x] **M7-V+** 宗门等级/十二档职位/十设施贡献闭环（战真结算仍 → M11）  
- [x] 化身独立宗门任务至少 1 条  
- [x] 宗门兑宠非法拒绝；白名单可兑  
- [x] 道友双向；交易行 + 拍卖 + 面交可用  
...
| --- | --- |
| 2026-08-14 | ARCH 对齐节 |
| 2026-08-13 | **出口打磨收口**：真正坊市、大厅导航、社交交易迁入、双修状态机、师徒日课/亲传等；§15 维持全勾；下一主线 → M8 |
| 2026-08-12 | **D26 Presence**：指针 [`玩家在线状态设计.md`](./玩家在线状态设计.md)；组队/面交/助战/道友改接统一门面 |
...
| 2026-08-12 | **M7-V+ 设施调整**：矿脉取消日领→被动入库+采矿挂机；灵药园兑换/托管；大阵权限分流与属性加点 |
| 2026-08-12 | **M7-V+**：§3.7～3.9 宗门等级/职位/十设施可玩骨架；建宗专精；战入口占位→M11；单测 `test_sect_org_ranks` / `test_sect_treasury_pages` |
