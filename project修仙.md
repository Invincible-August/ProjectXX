...
> 版本：v4.2
> 最后更新：2026-08-17
> 状态：**玩法框架定稿**（剧情细案与数值公式明确不设计；§3.3 雷劫范围 2026-08-06 修订为仅跨境；§11 灵宠自走棋硬顶 1 / §12.6 傀儡神识乘区 2026-08-17）
...
| --- | --- |
| 格子类型 | **本源**（一主）与 **旁支**（多副） |
| 初始数量 | 开局：本源 **1**、旁支 **2**；可用轮回点加旁支（软顶 7，无硬顶） |
| 镶嵌物 | 收集到的体质；同一体质装本源与旁支各一套效果 |
| 卸下 | 允许，有冷却或损耗（`TBD`） |
...
> **细则真相源**：[`化身系统设计.md`](./化身系统设计.md)（**v1.3** · **单化身** · 化身修士可突破 · 禁道主/轮回境 · 取消独战）  
> 本节只保留框架；数量、功能闸、体力、互传折扣、突破封顶以该文档为准。**加解锁档 / 改折扣只改数据。**
...
| 之后成长 | 化身独立挂机/修炼/分配突破；可超过凝练时的 50% 基线，甚至整体强于本体（仍受神识约束） |
| 境界突破权 | 化身自有大境界/小境/进度；分配突破复用本体规则（`actor=avatar`）。默认同一大境界内可超前小境，不可超本体大境界（`avatar_lead_majors` 默认 0）。**禁止**进入道主境、轮回境（身份/外环，非常规修为台阶）。雷劫只打本体。细则 [`化身系统设计.md`](./化身系统设计.md) **§1.6** |
...
| 渡劫 | 本体渡劫时化身不可上阵，可继续挂机；化身跨境 **不渡劫** |
| 轮回 | 化身解散或沉睡；轮回点商店可有 **化身专项** 强化。轮回结算 **不**把本体初始属性 / 成长属性套到化身 |
| 快照 | 防守快照可包含化身编成（必须含本体） |
...
- ~~化身是否独立跨境~~ — **已定案**：可突破，双封顶；禁道主境/轮回境（同上）
...
用灵宠扩展自走棋阵容与挂机辅助，同时用 **神识** 约束 **上阵那一只** 的强度（自走棋硬顶 1，避免堆宠）；与化身神识规则共用同一属性锚点。养成侧采用 **物种图鉴 + 个体品阶词条 + 技能装备**（见灵宠系统设计）。
...
| 上阵 | **自走棋硬顶 1 只**（穿戴栏 `pet` 格）；占 §7 棋子位；神识衰减这一只，不用「多宠按境界解锁」 |
...
| 规则 | 说明 |
| --- | --- |
| 神识池 | 上阵灵宠、化身（§10.5）与上阵傀儡共同消耗神识容量 |
...
- 制造可上阵傀儡棋子；消耗材料与维护灵石。
- 战损可能损坏，需修理。
- **持有 / 编成无件数硬顶**（禁止「只能带 1 只」的装备槽）；棋盘落子仍受 §7 总 `max_units` 与部署格约束。
- **战斗力由角色神识决定**：上场真傀消耗并入角色神识池（与化身、灵宠合计）；超量按总占用套公式，不硬拒上阵；公式见 [`M4双线程成长设计.md`](./M4双线程成长设计.md) **§6.4**。角色页编成板见 [`道具与装备实体设计.md`](./道具与装备实体设计.md) **§5.3**。
- 试炼木傀（教学）无背包行，不计入本乘区。
