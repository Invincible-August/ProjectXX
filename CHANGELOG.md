...
- **文档与 Agent 约束分目录**：设计稿迁入 `设计文档/`；规则与 Skills 放 `约束/` 并同步 `.cursor/rules`、`.cursor/skills`；根目录增加 `AGENTS.md` 与 `Cursor约束配置说明.md`。（2026-09-17）

- **Git 暂存纪律（§0.0.5）**：应纳入版本管理的新文件须同回合 `git add`；未要求不擅自 commit；写入开发计划与 `.cursor/rules/git-stage-new-files.mdc`。（2026-09-01）

- **物品 UI 图标框架（§0.0.4）**：目录/装备/功法/神通/体质预留 `icon`（缺省=定义 id）；`Item.get_icon()`；API 透出；前端 `ItemSlotVisual` 有资源出图、无资源出全名；样例 `assets/items/iron_sword_t1.svg`。（2026-09-01）

- **物品格展示规则（§0.0.4）**：有物品 UI（图标）优先显示图，无 UI 则显示中文名；预留 `icon`/`ui_key` 接口，不强制短名字段；写入开发计划与道具设计 §7.0。（2026-09-01）

- **物品品阶统一为粗糙～太古**：工坊造物 `quality` 与 §0.0.3 七档对齐（不再用凡品/良品/上品/极品）；权重表与装备展示同步；旧四档键读入兼容。（2026-09-01）

- **角色页装备稀有度展示（§0.0.3）**：穿戴槽/选择格用统一色条描边；悬停改为浅色框 + `RarityBadge`；装备 API 出 `rarity`/`rarity_label_zh`。（2026-09-01）

- **稀有度显示统一（§0.0.3）**：共用 `rarityDisplay.ts` + `RarityBadge`；列表芯片靠色、详情/选卡用中文徽章；已落到功法词条、灵宠列表/图鉴/详情、大道开道与道池图鉴；`设计文档/开发计划.md` 写入强制规则。（2026-09-01）

- **词条品阶命名**：灰白绿…改为粗糙/普通/优秀/精良/史诗/传说/太古；列表芯片不再写品阶字、只靠颜色；详情弹框带品阶色条与徽章。（2026-09-01）

- **词条升级消耗配置**：统一 `next_affix_upgrade_cost`（`affix_upgrade_cost[等级]` × 稀有度 `cost_mult`）；后台域 research 字段说明已补；默认消耗表加长；单测覆盖逐级上涨。（2026-09-01）

- **功法词条可读性**：品阶改为白底深字+色条描边，徽章实色白字；芯片不再塞消耗；悬停/详情标明「下次升级耗」随等级上涨。（2026-09-01）

- **功法词条品阶配色**：稀有度改为浅底深字，避免白/浅色字看不清；培养悬停改为品阶徽章+分行属性；详情弹窗同风格。（2026-09-01）

- **功法可修炼层数 / 大圆满**：目录与自创支持 `cultivable`；资源分配 1→10 层再入大圆满；五层与圆满叠奖励库 ATTR；自创定稿前双档三选；配置在 `techniques.yaml` / `research.yaml`（后台可改）。（2026-09-01）

- **功法升级页词条交互**：去掉甲乙说明与长文案；列表只显示名称/等级/消耗；悬停看效果，点击开详情后再升级。（2026-09-01）

- **事件日志本地持久化**：按角色写入 localStorage（约 24 小时 / 最多 200 条）；切页与刷新可恢复，登出或换角清空；研究室/工坊等页统一写入同一日志流。（2026-09-01）

- **功法基础加成次数**：标题去掉「皆可升级」，显示剩余可升级；每阶 `base_upgrade_cap` 为新增次数可后台配置；创建时法修读修为境界、体修读炼体对照境，按 `base_upgrade_create_bonus_per_rank` 每阶额外加成并冻结。（2026-09-01）

- **功法词条栏与突破成长**：创建时栏数按人物境界冻结（锻体 1…化神 4）；功法仍从锻体突破；突破若栏不足则补空位并可在升级窗推演；已有词条叠 `rank_boost`（`breakthrough_affix_bonus`），新位不叠。（2026-09-01）

- **仓库清理**：删除 Agent 临时目录 `.superpowers/sdd`（wip/brief/review 草稿）；新增根 `.gitignore`，不再跟踪日志、本地库、IDE 配置。（2026-09-01）

- **研究室已定稿分栏**：功法/阵法/符箓各自列表，不再混表；功法培养须点「升级功法」进入，列表与升级页分离。（2026-09-01）

- **功法词条稀有度**：粗糙→太古七档（原灰/白/绿…）；白绿蓝常见、灰紫次之、橙更少、红最低；越高初始属性与升级加成/消耗越高。目录与权重在 `research.yaml` technique_craft（后台域 research 可改）。（2026-08-31；命名 2026-09-01）

- **功法词条三选**：每效能至少 3 条占位词条（修为修炼不再只会抽到三个「周天」）；选中只高亮一项；栏位改称词条甲/乙。（2026-08-31）

- **功法阶中文与突破门槛**：前端不再露出 `true_immortal` 等内部 id（显示「真仙」）；升级点旁标明下一阶所需（如 `410 / 1200`）；`research.yaml` 已补全锻体→真仙各阶默认门槛，后台域 `research` 可改。（2026-08-31）

- **功法自研初始阶与废除**：定稿一律从锻体起须逐步突破（不随人物境界）；已定稿可「废除」——须先卸装，只删作者本门，秘籍学会/藏经阁/弟子副本保留。（2026-08-31）

- **功法培养态回流**：已定稿原创重登后正确显示升级点/基础加成/词条等级；突破钮展示下一阶所需升级点；发动条件加成栏位预留（后续后台可配词条/加成/属性）。（2026-08-31）

- **自研测试卡改后台发放**：【测】无限属性/效能正式卡改由运营后台「玩家管理 → 角色管理 → 发放自研测试卡」直发入包；不再挂在玩家端 GM「发材料」。（2026-08-31）

- **后台登录 bcrypt**：去掉 passlib，改为直接调用 `bcrypt` 验密/哈希，兼容 bcrypt 4.1+（修复 `__about__` 导致管理员无法登录）。（2026-08-31）

- **功法自研定稿与联调卡**：雏形展示改为内容摘要/甲乙丙篇（不用线性数字）；定稿名称全局唯一（私有+官方重名须重输）；GM「发材料」附带【测】无限属性/效能正式卡各一张。（2026-08-31）

- **功法自研 UI**：研究室改为「心法雏形 / 另开新篇」双栏工作台；镶嵌点格打开选择窗（属性/效能正式卡分流）；空白卡·类型卡·秘籍改在洞府储物使用，页内不再列背包卡片/秘籍。（2026-08-31）

- **工坊逐件制造**：`×N` 按件完成入包并递减剩余；取消排队项不重置正在制造的进度；取消按剩余件数比例退冻。（2026-08-30）

- **工坊队列直入包**：入队即冻材料/灵石/体力；支持数量与顺序排队；到期自动入包（无领取）；可取消退冻；体力按配方 `stamina_cost×数量`。（2026-08-30）

- **工坊结算时区**：`settle_jobs_async` 比较 `finish_at` 前先 `ensure_aware_utc`，避免 SQLite naive 与 aware 混比炸开工。（2026-08-29）

- **工坊联调配方**：炼丹/炼器/符箓/傀儡各加一条「【测】仅灵石」图纸（无材料、5 秒、扣 10 灵石；`stamina_cost: 0` 跳过体力）。（2026-08-29）

- **功法自研 P3**：藏经阁（秘籍上缴审核、条目学习 source=sect）；师徒不在本期。（2026-08-29）

- **工坊配方列表 500**：`GET /craft/recipes` 读符箓 `talisman_effect_id` 时配置解析缺字段，未处理异常在浏览器里显示成「无法连接后端」。已接回解析字段。（2026-08-28）

- **功法自研 P2**：原创者可制成秘籍；他人使用后学会只读副本（source=chance），书消失。藏经阁/师徒不在本期。（2026-08-28）

- **功法自研 P1**：研究室功法改为三步卡 + 多草稿 + 定稿培养；旧材料会话 `kind=technique` 返回 40201。GM 发工坊材料时附带空白卡 10 张。秘籍/藏经阁/师徒不在本期。（2026-08-28）

- **悟道入口下沉本尊属性**：角色页标题行只留「← 回大厅」与本尊/化身；悟道挂在本尊属性卡头右侧，仅本尊真仙可见。（2026-08-25）

- **角色页本尊/化身同页切换**：顶栏去掉「化身」；角色页标题「← 回大厅」后接本尊/化身切换，不整页重载。旧 `/avatar` 重定向到 `/character?actor=avatar`。（2026-08-25）

- **洞府去掉房间小钮**：枢纽已有工坊/研究室大卡，标题行右上不再放同名小按钮；换房间先回洞府再点大卡。（2026-08-25）

- **工坊迁入洞府**：工坊改为洞府二级页 `/cave/workshop`；顶栏去掉「工坊」，只留「洞府」。旧 `/workshop` 带 query 重定向。制造业 HTTP 仍为 `/api/v1/craft`。（2026-08-24）

- **工坊炼器部位 / 符箓功能筛选**：炼器配方栏增加部位筛选（武器、头部、胸甲、护手、腿甲、鞋履、饰品、法宝）；符箓增加功能筛选（增益、攻击、诅咒）。官方符箓挂 `talisman_effect_id`，领取写入 `meta.effect_id`。（2026-08-24）

- **工坊配方栏改表单**：配方卡内改为属性/等级筛选表单 + 表格点选（名称/属性芯片/制作等级）；先扩炼丹样本覆盖金木水火土风雷暗与 0～3 级门槛。后台域 `craft_recipes` 可表格/JSON 双写。（2026-08-24）
...
- **化身修士 OOP 重构（v1.3 代码）**：`CultivatorCharacter` 为后台按类渲染境界栏的父类；`AvatarCharacter` 不再挂 `SenseMinionCharacter`，改修士叶 + `DivineSenseConsumer`。编成永久必须含本体（吃独战）。`avatars` 自有境界字段与双封顶；化身面板不套用本体轮回 `initial_attr`/成长。单测 `test_game_arch_abc` / `test_avatar_realm_caps` / `test_avatar_features`。ARCH [`设计文档/统一实体类层次与重构方案.md`](./设计文档/统一实体类层次与重构方案.md) **v1.0.6**（2026-08-19）

- **化身 v1.3 设定冻结**（仅文档）：化身可突破（相对本体封顶）；绝对硬顶为 **道主境 / 轮回境**（非常规修为台阶）；轮回商店预留化身专项，结算 **不**把本体 `initial_attr` / 成长套到化身；取消独战。实现切片 **AVATAR-D07～D10**。真相源 [`设计文档/化身系统设计.md`](./设计文档/化身系统设计.md)。**M4-D02** 落入 D08；**M5-D07 取消**（2026-08-19）
...
- **M8+ 类型与实例整改（登记）**：类型=白板锚点、实例=最终属性；回收 24h + 待清理 7 天；清单见 [`设计文档/后续待完成.md`](./设计文档/后续待完成.md) §1.14；**拍板前不开 M9、不改正式专文**（2026-08-19）
...
- **选择栏紧凑化 + 统一悬停**：体质/功法/神通/装备点槽后用方格选装（不再一行一件）；悬停按装备属性固定 format。神通 i：「神通装备数量与修为和品阶相关」。[`设计文档/功法系统设计.md`](./设计文档/功法系统设计.md) **v0.2**；[`设计文档/道具与装备实体设计.md`](./设计文档/道具与装备实体设计.md) **v0.8.9**（2026-08-18）
- **神通池选装**：神通栏与功法同交互（点槽从已学池装备），无主副之分；格数 = 修为基数 + 跨境品阶。专栏 [`设计文档/神通系统设计.md`](./设计文档/神通系统设计.md) **v0.2**。单测 `test_divine_ability_loadout`（2026-08-18）
- **神通栏装备口子**：根基里神通与耐力对调；角色页右侧神通栏。专栏 [`设计文档/神通系统设计.md`](./设计文档/神通系统设计.md)。（2026-08-18）
- **角色面板排序**：悟性/耐力/神通进根基；战斗按资源→攻防→机动→抗性；生成属性按炼丹/炼器/制符成对。耐力=体修占位，神通=可装备格数（修为+品阶）。（2026-08-18）
- **角色面板分栏**：简介含轮回点与体力；「生活属性」改「生产属性」；天劫抗/心魔抗/吐纳效率进根基；根骨格不拉满。[`设计文档/ATTR战斗属性占位设计.md`](./设计文档/ATTR战斗属性占位设计.md) **v1.4.2**（2026-08-18）
- **悬停浮层字色**：功法/体质/傀儡说明/挂机拆解改为深色底浅色字，避免浅色 tooltip 把字压成纯黑（2026-08-18）
- **功法双角色装备口子**：主功法 1 格 + 技法多格（随修为增加）；悬停看名称/等级/属性/来源；有技能才在下方列出。玩家/NPC 带灵根，创角默认杂灵根。专栏 [`设计文档/功法系统设计.md`](./设计文档/功法系统设计.md)。单测 `test_technique_loadout`（2026-08-18）
- **大厅摘要去卡头**：去掉「角色摘要 / 打开角色页」一行；进角色页改走顶栏「角色」。`PlayNav` 含大厅…商店与账号，同一组 `size="small"` 按钮。元素抗并入「战斗属性」；开口异常抗 `resist_ailment`、暗抗 `resist_dark`。专栏 [`设计文档/战斗系统完善设计.md`](./设计文档/战斗系统完善设计.md)。单测 `test_combat_attrs` / `test_talisman_stack_and_status`（2026-08-18）
- **玩法壳全局跳转 + 装备栏上阵**：任意玩法页顶栏 `PlayNav` 可互跳。化身在装备栏开关上阵；灵宠/傀儡/符箓也在装备栏设置。神识只按装备栏占用（与棋盘落子无关）；阵法页只布阵。工坊按制作等级锁图纸并掷造物品质。符箓同种类不叠加与异常状态回合序冻结为纯函数（引擎完整链仍 M3-D03 / M8-D04）。专文 [`设计文档/符箓叠层与异常状态设计.md`](./设计文档/符箓叠层与异常状态设计.md)。单测 `test_loadout_sense_formation` / `test_craft_level_quality` / `test_talisman_stack_and_status`（2026-08-18）
...
- **M8 ITEM-0 丹药方案 A（设计）**：`use_effect` 白名单；时效多轨 `wall`（现实时间）/ `battle`（场次）/ `round`（回合）；禁止裸 duration；时钟结算仍 M8-D05。专文 [`设计文档/道具与装备实体设计.md`](./设计文档/道具与装备实体设计.md) **v0.8**（2026-08-17）

- **M8 ITEM-0 傀儡/灵宠兼容（设计）**：废止单指针傀儡槽；角色页 **17 区** = 15 装备 + 灵宠硬顶 1 + 傀儡编成板（无件数硬顶）；傀儡战力走 M4 §6.4 平行神识乘区。专文 [`设计文档/道具与装备实体设计.md`](./设计文档/道具与装备实体设计.md) **v0.7**（2026-08-17）

- **M8 ITEM-0 设计开篇（道具 ABC）**：[`设计文档/道具与装备实体设计.md`](./设计文档/道具与装备实体设计.md) **v0.7**（由 v0.6 修订）——17 区穿戴栏、福宝不可逆耐久、四页背包、占位不迁格；ITEM-0 优先于 R3（2026-08-17）
...
- **开 M8 设计（自研与内容管线）**：新增 [`设计文档/M8自研与内容管线设计.md`](./设计文档/M8自研与内容管线设计.md) **v1.0.1** / [`设计文档/M8前端目录与路由设计.md`](./设计文档/M8前端目录与路由设计.md) **v1.1**；竖切 R1～R6；路由 `/research`；ATTR-D02 必须先于自研；延后 M8-D01～D05；**前端补布局/样式文字方案**（1100px 玩法壳、mode-nav、虚线槽、BoardGrid、大厅不塞大卡）；开发计划 **v3.7**（2026-08-17）
...
- **统一实体类层次与重构方案（草案 v0.1）**：六大域 ABC（Character/Item/Content/Law/Environment/Org）+ Ability 授予模型 + 傀儡双切面 + 现行 `domain`/`services` 映射与 R0～R6 分期；文档 [`设计文档/统一实体类层次与重构方案.md`](./设计文档/统一实体类层次与重构方案.md)；登记延后项 **ARCH-R01**（2026-08-14）

- **双轨整改与配置兼容（草案 v0.1）**：步骤1存量按新 OOP 整改、步骤2未开发强制新规范；ContentStore 三模式（`yaml_authority` 测试默认 / `yaml_base_db_overlay` 现行 / `db_authority` 正式目标）；保证测试继续只改 YAML；文档 [`设计文档/双轨整改与配置兼容方案.md`](./设计文档/双轨整改与配置兼容方案.md)；**ARCH-R02**（2026-08-14）
...
- **ATTR v1.3 叠层封装 + 计划收口**：`domain/combat.py` 引入 `AdditiveSource` / `apply_aliases` / `public_combat_final_summary` / `engine_unit_core_from_final`；`CombatCalculator` 与面板同源；道友卡修正为 `magic_atk`/`magic_def`（消灭 `mag_atk` 分叉）；[`设计文档/ATTR战斗属性占位设计.md`](./设计文档/ATTR战斗属性占位设计.md) **v1.3**；[`设计文档/开发计划.md`](./设计文档/开发计划.md) **v3.2** / [`设计文档/后续待完成.md`](./设计文档/后续待完成.md) §0.1：M7 出口打磨收口，下一主线明确 **M8（ATTR-D02）**（2026-08-13）

- **玩法壳 WS 长连接**：壳内切页不再反复「正在连接仙界…」；大厅首屏欢迎只一次；WS `connect` 幂等保活，登出/离开玩法壳才断开；重连后自动重订聊天房（2026-08-13）

- **师傅传授徒弟事件日志**：`POST /mentor/teach` 后徒弟在线则 WS `game.log` 直推大厅事件日志；离线写入 `pending_event_logs`，领取离线收益时一并展示并清空；无离线 pending 时前端 `POST /characters/me/event-logs/ack`（2026-08-13）

- **灵石/炉鼎时限整型校验**：交易面交灵石与炉鼎小时前后端均强制非负整数；一口价/拍卖/邮件灵石同规则；小数与布尔拒绝（2026-08-13）

- **亲传变更写入事件日志**：保存亲传时逐条写入「已更新亲传弟子（n/3）」「某某被设为亲传弟子 / 已解除亲传」；出师自动解除亲传同样入日志（2026-08-13）

- **亲传冷却与出师解除**：出师自动解除亲传；指定后隔日方可解除，解除当日不可再指定同一人；`direct_disciple.cooldown_days`（2026-08-13）

- **师承单与亲传弟子**：师徒页展示师承单（含已出师，按拜师序标大/二/三弟子）；师傅可设最多 3 名亲传；亲传授业/解惑日课次数各 +1，传道与传授次数不变；`POST /mentor/direct`；单测覆盖（2026-08-13）

- **社交日志同步大厅 + 徒弟请学**：社交各面板 `@log`（含师徒传功/请学数值）写入共享 `gameLog` store，回大厅可见；徒弟每日可指定师傅已掌握功法请学一次，可叠加师傅未学完的同种传授进度；`POST /mentor/study`；`mentor.yaml` `study`；单测覆盖（2026-08-13）

- **邀请通知去重 + 倒计时时区**：组队不再因 invite+update 双弹；交易/双修仅关键事件弹窗；过期时间统一 UTC `Z`，前端按 UTC 解析，修复一点开就「已超时」（2026-08-13）

- **邀请通知点击跳转**：右上角交易/组队/双修/道友/道侣提示可点进对应社交子页；组队补弹窗通知；统一可点样式（2026-08-13）

- **修复交易邀请接受按钮不及时出现**：邀请 WS 改为 DB 提交后再推；前端乐观写入待接受列表并短重试拉取；点通知先刷新/加载会话再进交易页（2026-08-13）

- **修复重新登录停在上次页**：登录/退出/根路径分流不再回跳「上次玩法页」或 `redirect`；重新登录固定进大厅（创角/引渡/渡劫状态除外）（2026-08-13）

- **交易界面打磨**：道具格对齐邮件尺寸；去掉重复刷新与「更新草稿」（改自动同步）；页内待接受列表；未成交不展会话台；去掉会话#/版本号/在线文案；点通知进交易页后手动接受（2026-08-13）

- **修复交易锁定被对方改价拆掉**：未锁定方更新草稿不再清空/退回对方已锁定托管；面交 WS `session` 改为接收方视角（`you_are` 不再串角色）（2026-08-13）

- **师徒玩法优化**：收徒须高徒 ≥1 大境界；弟子追上师傅大境界自动出师；日课三选一（传道灌修为/炼体度、授业制造业功法、解惑功法等级，数值 `min(弟子需求100%,师傅池10%)`）；另每日可传授功法/丹方图纸并按阶梯多日累计；API `POST /mentor/lesson` · `/teach`；表 `mentor_transmissions` / `character_craft_knowledge`；单测补自动出师与传授（2026-08-13）

- **引渡救援三类名单**：社交·引渡改为「普渡众生 / 同门引渡 / 亲友引渡」；分别列出待引渡道友、同门、以及道友+道侣+师徒+炉鼎；`GET /ferry/rescue-targets`；`POST /ferry/rescue` 增 `mode=kin`（2026-08-13）

- **交易迁入社交 + 大厅邀请列表**：道友交易从商店/拍卖行剔除，改名「交易」进 `/social?mode=trade`；页面同邮件写信台（右侧道友/师徒/宗门/道侣快捷选人 + 道号；4×4=16 格点空开背包、点满移除）；收邀 WS `face.invite`/`face.update` 弹窗；`GET /trade/face/pending` · `invite-options`；大厅日志下方邀请列表（类型/发起人/接受拒绝/剩余时间，终态消失）；邮件附件点满格仍为移除（2026-08-13）

- **面交可要约炉鼎**：报价除物品/灵石外可「愿为对方炉鼎」并设现实小时；双方至多一侧可要约；已是主人不可再献身为对方炉鼎，已是其炉鼎可延长；**互为道侣不可互为炉鼎**（道侣仍可为他人炉鼎）；一炉一主；到期惰性解除；仅主人可随时解除；列表展示到期时间（2026-08-13）

- **战斗体力进角色摘要**：`GET /characters/me` 增 `battle_stamina`（惰性恢复 left/cap）；生活属性标签改为「战斗体力」；大厅角色摘要与角色页展示（2026-08-13）

- **双修索取模式（蛇蝎）**：新功法 `serpent_extract`（mode=extract）；被索取修为过低→初始转化率 0，索取方过低→初始为负；双修开始扣双方战斗体力（双增相同、传功传方>受方、索取索取方>被索取）；`stamina_costs`/`climax`/功法等均可管理后台改（2026-08-13）

- **双修流程对齐**：道侣邀约/宽衣 60s 超时终态；受邀方离开双修台取消宽衣；炉鼎可接受不可拒、超时自动接受/自动宽衣；开始后后台高潮循环结算并按秒上榜后自动解除；WS 弹窗提示（点进双修台）+ 倒计时归零立即刷新（2026-08-13）

- **双修状态机重做**：邀约→接受→宽衣→开始（高潮循环自动结算）；炉鼎不再强制接纳，超时自动接受/宽衣；WS `dual.invite` / `dual.update`；API `POST /dual/{id}/undress|start`；前端移除掷骰/手动结算（2026-08-13）

- **双修门闸与选人 UI**：道侣/炉鼎右侧选人对齐邮件布局；面交仅「进行中」挡双修（成交/取消/过期终态不挡，检查前惰性过期）；忙时文案改为「正忙，暂不可双修」；队伍/团队/秘境亦挡；炉鼎终态强制接纳，接纳/强制时修炼中自动停修（2026-08-13）

- **道友关系单页**：大厅去掉独立「道友」按钮；社交内道友/道侣/炉鼎同页分区（我的道友/道侣/炉鼎；若为他人炉鼎则显示「我的主人」）；道号后直接「申请道友/申请道侣」；道友可一键申请道侣、道侣可一键申请道友；炉鼎仅「解除炉鼎」；道侣申请 WS 提示同道友（`bond.request` / `bond.update`）（2026-08-13）

- **双修社交优化**：Tab 改为「双修台 / 时长榜」；禁止手填道号，仅从道侣/炉鼎右侧选人；道侣可接受拒绝，炉鼎在线且空闲强制接纳；道友页增道侣/炉鼎类目（炉鼎暂不可邀请添加）；榜改为累计双修秒数前 100 + 角色位时长分榜；传功/双增按时长与修为差距结算（转化可超 100% 或为负）；API `/bonds/*`；单测 `test_dual_cultivation_dice_ranks` / `test_dual_transfer_duration`（2026-08-13）

- **道友拜帖系统提示**：申请时对方收到「新的道友拜帖」通知；接受/拒绝后发起方收到同意/拒绝提示，并同步刷新道友列表（WS `friend.request` / `friend.update`）（2026-08-13）

- **修复登录后总进大厅**：带 query 的 `redirect`（如 `/social?mode=party`）曾被整段塞进 `path` 导致匹配失败；现正确拆 path/query。退出/会话恢复会记住上次玩法页并回跳（2026-08-13）

- **修复组队邀请名单在线不一致**：道友用真实 Presence、宗门/师徒曾吃 `party_dev_assume_online` 全标在线；现三端均用真实在线；点离线提示「道友不在本界」；`party_dev_assume_online` 默认关（2026-08-13）

- **队伍/团队优化**：去掉「打开队伍聊天」；邀请改为道友/宗门/师徒快捷选择+道号（右侧轨布局同邮件）；邀请 60 秒超时，列表绿接受/红拒绝+倒计时；队伍≤5，可转团队≤40，人数≤5 可转回队伍；团队仅团队秘境/团队·野外 Boss/势力争夺，普通秘境禁止，非团队 Boss 无掉落修为（门闸 `party_rules`）；API `GET /party/invite-options`、`convert_to_team` / `convert_to_party`（2026-08-13）

- **邀请化身（助战重做）**：道友按钮改为「邀请化身」；开关迁到化身页（开=可邀入队，关=「化身正在闭关中」）；独立助战体力槽（不受探索体力/加成，仅随境界变容，归零须恢复至 20%）；同时仅一会话（忙=「化身正在助战中」）；PVE 战后自动离队，秘境整场结束离队；单测 `test_avatar_friend_assist_pve`（2026-08-13）

- **修复误弹离线收益**：离线判定原只看 `last_settled_at` 缺口 ≥5 分钟；离开大厅会停 sync，切页/切后台易误判。现改为：① 挂机 sync 挂在玩法壳（离开大厅不停）；② WS Presence 仍在线时长缺口带帽直接入账、不写 pending；真关浏览器/断 WS 后仍弹领取（2026-08-13）

- **道友社交优化**：去掉寄信按钮；在线/离线文案；离线点私聊/组队/交易提示「道友不在本界」；列表在线改为真实 WS Presence（`friends.dev_assume_online` 默认关）；可查看道友修为/功法/属性（在线实时、离线快照）；隐私开关遮掩则天机提示（2026-08-13）

- **邮箱写信台 UI（魔兽风）**：道友/宗门/弟子改到右侧按钮，弹窗选人；附件固定空栏位；「背包」打开点选入栏；可堆叠物品点击弹出附着数量窗（2026-08-13）

- **邮件系统增强（并入赠送）**：一键领取（后标已读）/一键已读/一键删除（仅已读且附件已领）；发信可附灵石与最多 6 种道具（数量≤最大堆叠，ADM 可调 `max_attachment_lines`）；道友/宗门/弟子快捷选人；宗门群发（掌门及以上）与弟子群发（师傅）；标题无默认值仅占位提示；社交「赠送」入口移除并入邮件；`POST /gifts` 兼容转发（2026-08-13）

- **测试服运维文档**：根目录新增 [`设计文档/测试更新指令.md`](./设计文档/测试更新指令.md)（conda + screen + Nginx `:8080` / 后端 `:8100`；完整更新与按范围精简步骤）（2026-08-13）

- **修复前端生产构建类型错误**：`BazaarPanel` 为 `el-input-number` 回调补 `number | undefined`；`useActivityGate` 去掉未使用的 `isProductiveDirection` 导入（2026-08-13）

- **挂机单位文案**：玩家可见「片」统一改为「周天」（本周天预计、+/周天、结算 N 周天等）（2026-08-13）
- **角色摘要收敛**：合并「状态/双线程」为「本体/化身」一行；修炼互斥说明仅留在修炼区，摘要不再重复（2026-08-13）
- **修炼区文案与信息精简**：按钮「修炼/淬体/制造业修炼」；进行中不再显示预估速度与实时推算，仅保留环境修正行（2026-08-13）
- **修炼区加成信息收敛**：环境修正仅保留「基础→有效」与总乘区；乘区拆解与时辰/天气说明改到旁侧 i 悬停提示（2026-08-13）
- **大厅再收敛**：去掉战斗栏与时辰说明条；时辰/天气说明改到顶栏悬停时辰弹出；活动状态与双线程摘要并入角色摘要；修炼区标题行醒目「资源分配/进阶」入口；顶栏增加化身/账号；账号页承载资料·改密·退出；化身挂机支持宗门采矿（计入名额，灵石入本体）（2026-08-13）

- **大厅导航收敛 + 真正坊市**：去掉调参 DEV / 快速教学战 / M4～M7 侧栏门闸；顶栏保留角色·布阵·战斗·宗门·社交·道友·商店。社交中心纳入道友/队伍/双修；商店中心承载坊市（NPC 固定货架买卖灵石）/ 拍卖行（原交易行+竞拍+面交）/ 天道商店。API `GET/POST /trade/bazaar*`；单测 `test_bazaar`（2026-08-13）

- **淬体层/期对齐主修**：炼皮/锻骨为 1～10 层，通脉起为初/中/后/圆满；淬体有成功率、失败回退进度、不渡劫；道体 `next_major` 预留扩境（2026-08-12）

- **淬体命名与进阶栏合并**：淬炼改称淬体；资源分配支持投入境界 / 投入淬体 / 升级功法（炼体功法自动扣淬体度池）；修为突破与淬体同栏切换（2026-08-12）

- **淬炼 vs 修为突破**：炼体境晋级改为显式「淬炼」（`GET/POST /quench/*`），挂机只涨进度不自动晋级；大厅「修为突破」与「淬炼」分栏（2026-08-12）

- **炼体九境**：炼皮→锻骨→通脉→换血→炼脏→玉髓→金身→法身→道体，对照主修锻体→大乘解锁；挂机炼体涨进度，满且主修达对照境可晋级；角色面板「炼体程度」展示本境（2026-08-12）

- **角色摘要与挂机计时**：摘要去掉攻/血（移详情）；摘要展示炼体度/炼体程度/制造业经验；修炼与采矿自点击起重新计满一段 tick，非墙钟整分（2026-08-12）

- **采矿日志与环境加成**：采矿结算写入大厅日志；个人灵石套用时辰/天气/通道；修炼区 IdleEnv 随世界轮询实时刷新（修灵/炼体/制造业/采矿同理）（2026-08-12）

- **采矿整分钟结算 + 角色页**：采矿/宗门库按分钟对齐；首分钟不扣体力，结算时灵石与体力同步；修炼区采矿进度条；新增 `/character`（左属性折叠、右体质装备+功法）；大厅改为角色摘要（2026-08-12）

- **采矿挂机与修炼区对齐**：采矿占用修炼互斥态并在修炼区展示；区中无开始采矿入口，仅采矿中显示「结束采矿」；去掉修灵/炼体/制造业与「停止修炼」的重复停止按钮（2026-08-12）

- **宗门设施玩法调整（矿脉/灵药园/大阵）**：矿脉取消日领，产出持续入宗门灵石库；采矿挂机占名额、耗体力得个人灵石并加速入库；灵药园支持直接兑换与托管种植；宗门大阵无管理权仅兑换/上缴，有权者可选阵/启停/属性加点（2026-08-12）

- **开发计划 v3.1 · 延后项镜像入里程碑**：[`设计文档/后续待完成.md`](./设计文档/后续待完成.md) 全部「待做」项归入 [`设计文档/开发计划.md`](./设计文档/开发计划.md) **§1.2 计划队列**及 M3～M13 / ADM 各节「待做队列」；§20 下一主线改为 **M8（ATTR-D02）**；登记册 §0.1 同步（2026-08-12）

- **ATTR-D01 落地（schema 占位竖切）**：`combat_attrs.yaml` + ADM 域；`build_combat_attrs` / `LifeAttrBlock`；`CharacterPublic.combat`+`life` + `GET /characters/me/combat`；大厅 `CharacterPanel` 分栏与来源拆解；`atk`=`phys_atk` 别名兼容；单测 `test_combat_attrs`；设计见 [`设计文档/ATTR战斗属性占位设计.md`](./设计文档/ATTR战斗属性占位设计.md) **v1.2**（2026-08-12）

- **ATTR 占位设计 v1.1（仅文档）**：[`设计文档/ATTR战斗属性占位设计.md`](./设计文档/ATTR战斗属性占位设计.md)——实体扩至玩家/化身/灵宠/傀儡/NPC/怪/Boss/宗门大阵；战斗键拆物法攻防+命中闪避+七维抗性+力敏智悟根；新增 `LifeAttrBlock`（体力/心魔天劫抗/吐纳/耐力/灵巧/精密/心性）；`atk`/`defense` 别名锁定；开发计划 §0.6.2 / README / 后续待完成同步（2026-08-12）

- **宗门系统深化（M7-V+ 可玩骨架）**：宗门九档等级（草庐→道庭）+ 十二档弟子职位/任命/次日自升；十设施（议事厅/藏宝阁/藏经阁/三工坊/大阵/矿脉/灵药园）贡献闭环；建宗必选专精；势力战入口占位→M11；`sects.yaml`/ADM schema 扩展；单测 `test_sect_org_ranks` / `test_sect_treasury_pages`（2026-08-12）

- **宗门 UI 调整**：总览只读等级/设施；升等与增益迁入议事厅（权限可见）；工坊更名锻造工坊/炼丹阁/服务工坊，支持上缴与兑换图纸（2026-08-12）

- **大厅私聊入口**：主页「私聊」按钮打开 `DmDialog`；角标为有未读的**会话数**（`dm_unread_peers`，非消息条数）（2026-08-12）

- **私聊持久化 + 独立弹窗**：私聊不受会话级清空；每会话保留最近 `dm_history_limit`（默认 100，ADM 可配）并支持清空；ChatDock 去掉私聊页签；道友/坞角标打开 `DmDialog`；`POST /chat/dm/clear`；单测 `test_chat_dm_persist`（2026-08-12）

- **玩家在线状态（Presence）**：统一 `PresenceService`（WS 鉴权连接 + 可配 `grace_sec`）；组队/道友/面交/助战/赛会改接 `is_online_for`；上线/离线推送 `presence.changed`（道友+队友）；设计见 [`设计文档/玩家在线状态设计.md`](./设计文档/玩家在线状态设计.md)；多 worker → **PRESENCE-R01**（2026-08-12）
...
- **ATTR 占位设计（仅文档）**：[`设计文档/ATTR战斗属性占位设计.md`](./设计文档/ATTR战斗属性占位设计.md) **v1.0**——锁 `CombatAttrBlock` / 叠层 / 面板 `breakdown`；澄清 **M13=填正式曲线、不重设计字段**；**ATTR-D01 → 设计中**；开发计划 §0.6.2 同步（2026-08-11）

- **开 M7 设计（仅文档）**：[`设计文档/M7宗门社交与经济设计.md`](./设计文档/M7宗门社交与经济设计.md) / [`设计文档/M7前端目录与路由设计.md`](./设计文档/M7前端目录与路由设计.md) **v1.0**——五路由（`/sect` `/market` `/social` `/dual-cultivation` `/shop`）+ ChatDock 横切；竖切 L1～L8；锁双修 D-WD1～4；CHAT/HERITAGE 细则并入核心设计；登记 **M7-D02～D05**；本轮无业务代码（2026-08-11）
...
- **排期**：下一主线曾定为 M6 打磨包；**打磨包已收口**，其后开 M7；`设计文档/后续待完成.md` §0.1 / 开发计划 U6 已同步
...
- **定案（仅文档）**：道主有主更替 → **道主之争赛会**（报名 / 同道淘汰 / 道主在线双模决战 / 半决决赛直播）；见 [`设计文档/M6大道与道主设计.md`](./设计文档/M6大道与道主设计.md) v1.1 §5、[`设计文档/后续待完成.md`](./设计文档/后续待完成.md) **M6-D06**（P1～P4）；本轮无业务代码（2026-08-10）
...
- **M6 前端骨架落地（2026-08-10）**
  ...
  - 类型 / API / Pinia：`dao` · `daoLord` · `worldEvents` · `ws`；组件与视图按 [`设计文档/M6前端目录与路由设计.md`](./设计文档/M6前端目录与路由设计.md)
...
- **M6 设计开篇（2026-08-10 · 仅文档）**
  - [`设计文档/M6大道与道主设计.md`](./设计文档/M6大道与道主设计.md) **v1.0**：板块 S/T/U；竖切 **W1～W6**；决策 D1～D16；样本开道/道池、WS 协议、道主挑战（实时挑战者×快照道主）、Boss/秘境骨架
  - [`设计文档/M6前端目录与路由设计.md`](./设计文档/M6前端目录与路由设计.md) **v1.0**：新增 `/dao` `/dao-lord`；`src/ws` 客户端；大厅 `HallDaoGate`
  - `设计文档/后续待完成.md`：新增 **§1.11 M6-D01～D07**；**M4-D05 / M5-D04 / M1-D33 → 设计中**；§0.1 指向 M6 实现
...
- **突破取消读条（2026-08-10）**
  ...
  - **文档**：[`设计文档/异步真读条突破设计.md`](./设计文档/异步真读条突破设计.md) **v1.2**
...
- **M5-D05 / M1-D20 异步真读条突破设计定案（2026-08-10 · 仅文档）**
  - **新建**：[`设计文档/异步真读条突破设计.md`](./设计文档/异步真读条突破设计.md) **v1.0**——假读条→服务端跨请求 `breaking_through`；开读条/进度/懒结算；与渡劫会话分离；配置 `async_channel` + ADM；Feature Flag 可回退同步 attempt
...
- **§0.0.2 中文信条 + M3-D07 设计定案（2026-08-10 · 仅文档）**
  ...
  - **新建**：[`设计文档/四象禁制与环境天气载荷设计.md`](./设计文档/四象禁制与环境天气载荷设计.md) **v1.0**——禁制三子类（禁远程物理/法术/全部远程）、环境·天气 `combat` catalog 载荷、与世界天气边界、战报中文渲染、样本阵与验收单测表
...
- **嘲讽光环封装 + 挂账标注（2026-08-10）**
  ...
  - **文档**：[`设计文档/开发计划.md`](./设计文档/开发计划.md) §5 增 M3 打磨对照表；[`设计文档/后续待完成.md`](./设计文档/后续待完成.md) 新增 **M3-D06b**（Phase B → 挂 **M3-D03**）；设计 **v1.1.2**
...
- **嘲讽光环设计定案（2026-08-10 · 仅文档）**
  - **新建**：[`设计文档/嘲讽光环设计.md`](./设计文档/嘲讽光环设计.md) **v1.0**——静态 YAML 光环注册表、按侧 `taunt_aura_mask`、进入触发 `STOP_TAUNT`、决策层嘲讽短路、死亡解除、战报 `taunt`/`ai_retarget`、ADM 域 `taunt_auras`；技能时长/驱散 → Phase B（M3-D03）
...
- **战斗友军占位协调（2026-08-10）**
  ...
  - **文档**：`设计文档/M3自动移动攻击算法设计.md` **v2.5** §5.4 修订
...
- **阵法部署与自研设计定案（2026-08-07 · 仅文档）**
  - **新建**：[`设计文档/阵法部署与自研设计.md`](./设计文档/阵法部署与自研设计.md) **v1.0**——部署四模式（`default`/`fixed`/`free_own`/`mask`）、地形 `brush` 仅定稿时落笔、`force_shifts`、ADM/自研同 schema、Phase A（M3-D08）/ Phase B（M8）竖切
...
- **化身定案：取消多化身 + 细则文档（2026-08-07 · 仅文档）**
  - **新建**：[`设计文档/化身系统设计.md`](./设计文档/化身系统设计.md) **v1.0**——永久单化身；境界功能矩阵；体力/日行动；互传保留率；独战/探索/任务竖切；对齐 §0.0 / §0.7
  - **取消**：原 **M4-D01 多化身**；`max_avatars` 必须为 `1`
  - **拆项**：`AVATAR-D01～D06` 登记于 `设计文档/后续待完成.md`；§0.1 P1 改为 AVATAR-D01
...
- **ADM 双写 + 中文字段 v1.3.3（2026-08-07）**
  - 强制规则写入 [`设计文档/开发计划.md`](./设计文档/开发计划.md) **§0.0.1** / [`设计文档/后台管理系统开发计划.md`](./设计文档/后台管理系统开发计划.md) **§8.2**：**表格 + JSON** 双写；字段须有中文 `label_zh`/`help_zh`；表格由后端 format 为域 JSON
...
- **ADM 后台管理系统骨架落地（2026-08-07）**
  ...
  - **文档**：[`设计文档/后台管理系统开发计划.md`](./设计文档/后台管理系统开发计划.md) **v1.3**；[`设计文档/开发计划.md`](./设计文档/开发计划.md) **v2.0**；`后续待完成` M2-D01/ADM 骨架已消化；README 启动说明
...
- **开发计划 v1.9 · ADM 最高优先级（2026-08-07 · 仅文档）**
  - [`设计文档/开发计划.md`](./设计文档/开发计划.md) 新增 **§0.0 强制提示词**：凡可配置需求必须适配后台管理系统
  - 里程碑总览 / **§1.1 ADM** / §20 开工顺序第 1 条升为 **P0**
  - [`设计文档/后台管理系统开发计划.md`](./设计文档/后台管理系统开发计划.md) **v1.1** 取消后置；与主计划同步
  - `后续待完成` / README 同步

- **后台管理系统立项（2026-08-07 · 仅文档）**
  - 新建 [`设计文档/后台管理系统开发计划.md`](./设计文档/后台管理系统开发计划.md) **v1.0**：独立部署；覆盖灵兽/功法/宗门/地图/怪物/道具/天气等全站内容域；竖切 ADM-0～10；挂 M2-D01
  - [`设计文档/后续待完成.md`](./设计文档/后续待完成.md) 登记 **ADM**；灵宠设计 §1.4 / 开发计划 §20 / README 指针同步

- **灵宠 v1.2：热插拔 + 并行不冻结（2026-08-07 · 仅文档）**
  - [`设计文档/灵宠系统设计.md`](./设计文档/灵宠系统设计.md)：§1.4 热插拔强制；catalog 派生注册表；灵兽宗 **API 先行**（`/pets/sect/affix/*`）；PET/野外 **不绑** M7/M9
  - [`设计文档/后续待完成.md`](./设计文档/后续待完成.md) / M4 §7.4.3 / README 同步

- **灵宠对战模式定案（2026-08-07 · 仅文档）**
  - [`设计文档/灵宠系统设计.md`](./设计文档/灵宠系统设计.md) **v1.1** §10：PET-D05 = 神奇宝贝式**简易回合制**（选招→比速→结算）；明确**不**复用自走棋棋盘/AP/移动
  - GDD §11.3、[`设计文档/后续待完成.md`](./设计文档/后续待完成.md) PET-D05 验收钩子同步

- **灵宠系统愿景与分期（2026-08-07 · 仅文档）**
  - 新建 [`设计文档/灵宠系统设计.md`](./设计文档/灵宠系统设计.md) **v1.0**：宝可梦式遭遇/图鉴、个体品阶、暗黑词条（最多 9）、四技能装备、捕获骰、灵兽宗改类型、1v1；与自走棋边界写死
  - GDD §11 改为框架 + 指针；M4 §7.4 **N4 收窄**（`grade`/catalog 骨架；真词条/技能不进 N4）
  - [`设计文档/后续待完成.md`](./设计文档/后续待完成.md) 拆 **PET-D01～D06**；M4-D04c 加厚；§0.1 P0 备注同步
...
- **M5 优先打磨收口 + 下一会话并行包（2026-08-07）**
  ...
  - **文档**：[`设计文档/后续待完成.md`](./设计文档/后续待完成.md) 新增 **§0.1 下一会话开工包**（★ N4 / M4-D01/D03 / M3-D06～D08 / M5-D05）；开发计划 §20 同步

- **M5 完成度核对与计划同步（2026-08-07）**
  - 结论：主路径 E1～E6 / O–R **已收口**；收口后强化（永久加成/轮回袋/新生商店/阵法 reset·prune/筑基前免费挂机）已落地
  - 文档：`设计文档/开发计划.md` v1.7；外环设计 v1.4；前端目录 v1.3；挂机/骰子设计各 v1.1；`设计文档/后续待完成.md` 增 **M5-D11**（切段 settle）· **M5-D12**（突破 dice UI）；延后范围 **D01～D12**
  - README 索引版本号同步
...
- **文档对齐修为骰子（2026-08-06）**：`设计文档/开发计划.md` **v1.6 §0.9**；`设计文档/M5环境与轮回外环设计.md` **v1.3**；`设计文档/M5前端目录与路由设计.md` **v1.2**（突破/遮天展示纪律与服务端 `dice` 字段）

- **修为骰子系统（2026-08-06）**：境界 YAML 查表上下限 + 功法/体修/气运修正；突破/战斗先攻·伤害·阵法接入；制造失败/遮天/品阶权重走统一 RNG 门面。设计见 [`设计文档/骰子系统设计.md`](./设计文档/骰子系统设计.md)；配置 `dice.yaml`；延后 **DICE-R01～R03**

- **挂机速率按境界 + 内外部加成骨架（2026-08-06）**
  - **公式**：`effective = floor(realm_base × channel_mult × env_mult)`；设计见 [`设计文档/挂机速率与加成设计.md`](./设计文档/挂机速率与加成设计.md)
...
- **开发计划 v1.4 · 显性设计原则（2026-08-06）**：`设计文档/开发计划.md` 增补 **§0.7**——「研究游戏设计」为本玩法之一，禁止隐性数值/黑盒修正；配置须带 catalog 说明字段；新会话/新板块按显性验收清单交付
...
- **M5 后端落地（2026-08-06）**：按 `设计文档/M5环境与轮回外环设计.md` v1.1 竖切 E1～E5（+ E6 联调）
...
- **M5 前端落地（2026-08-06）**：按 `设计文档/M5前端目录与路由设计.md` v1.1 实现环境与轮回外环 UI
...
- **M5 渡劫玩法加厚 v1.1（2026-08-06，设计-only）**：
  ...
  - `设计文档/后续待完成.md` 增 **M5-D08～D10**；文档：`设计文档/M5环境与轮回外环设计.md` / `设计文档/M5前端目录与路由设计.md`

- **M5 设计开篇（2026-08-06）**：
  - [`设计文档/M5环境与轮回外环设计.md`](./设计文档/M5环境与轮回外环设计.md)：六时权威时钟、区域天气池与三锁点、环境修正切片、雷劫/渡劫会话、待引渡与轮回保留表、剧情「已历可跳」flag；竖切 **E1～E6**；决策 D1～D12
  - [`设计文档/M5前端目录与路由设计.md`](./设计文档/M5前端目录与路由设计.md)：新增 `/tribulation` `/reincarnation`；全局 `WorldClockBar`；大厅 `HallEnvGate`；待引渡路由守卫
  - `设计文档/后续待完成.md`：新增 **§1.7 M5-D01～D07**；**M1-D21 → 设计中**；M1-D20 挂 M5-D05
  - `设计文档/开发计划.md` §7 / README：挂上 M5 设计索引；近期开工顺序改为按 E1～E6 实现
  - **实现未开**；不重开 M4 出口

- **开发计划 v1.3（2026-08-05）**：M7 增补 **双修**（板块 WD）——双修功法决定「双人同增」或「单向传修为」；会话内掷骰定效果档与时长；男修/女修各自维护 **一号榜** 与 **零号榜**；创角性别字段（WD0）；化身双修延后为 **M7-D01**；M7 日历约扩至第 21～26 周；`设计文档/后续待完成.md` / README 同步
...
- **灵宠物种与获取途径正式规划（2026-08-05）**：文档-only——`设计文档/M4双线程成长设计.md` §7.4 种族/物种、§7.5 途径矩阵与 N4/N5；`设计文档/开发计划.md` 增 N4/N5 及 M5 轮回带宠 / M7 宗门兑宠 / M9 野外捕获指针；`设计文档/后续待完成.md` **M4-D04 拆 a/b/c**；前端目录灵兽园 IA 与进度表同步；**实现未开**，不重开 M4 T1～T6 出口
...
- **开发计划 v1.2（2026-08-05）**：M7 社交经济补全——**聊天室**（世界/宗门/私聊+WS）、**邮件与赠送**、**师徒**、**面对面交易**；交易行/拍卖原 W2 保留；出口标准与日历（约第 21～25 周）同步；`设计文档/后续待完成.md` §3 更新

- **开发计划 v1.1（2026-08-05）**：将「之后」拆为 **M8～M13** 并写入步骤表——自研功法/阵法/符箓（M8）、地图·势力·静态 NPC（M9）、奇遇·任务·剧情管线（M10）、世界 Boss/宗门战/势力战/抢地盘（M11）、AI NPC + AI 天道化身第三方 API（M12）、内容填充与数值案（M13）；原则补充剧情/AI 后置；`设计文档/后续待完成.md` §3 同步；GDD 仍不写细案（工程排系统）

- **M4 设计开篇（2026-08-05）**：
  - [`设计文档/M4双线程成长设计.md`](./设计文档/M4双线程成长设计.md)：化身双挂机、工坊配方、灵宠雏形、神识占位；竖切 **T1～T6**；决策 D1～D10
  - [`设计文档/M4前端目录与路由设计.md`](./设计文档/M4前端目录与路由设计.md)：新增 `/avatar` `/workshop` `/pets`；大厅 `HallDualThreadGate`；Bench/阵法锁扩展
  - `设计文档/后续待完成.md`：新增 **M4-D01～D06**；M3 打磨项标明不阻塞 M4 出口
  - `设计文档/开发计划.md` §6 / README：挂上 M4 设计索引；前端分步表标「设计中」

- **M3 延后项文档整理（2026-08-05）**：`设计文档/后续待完成.md` 校正 M3-D03 目标阶段，新增 **M3-D06～D10**（嘲讽/四象加深/部署扩展/report_mode/算法打磨）；`设计文档/M3战斗成型设计.md` **v1.5**、算法文档 **v2.4** 只保留现行已落地内容，未做细则迁入登记册
...
- **M3 设计开篇（2026-08-04）**：
  - [`设计文档/M3战斗成型设计.md`](./设计文档/M3战斗成型设计.md)：7×7、布阵、阵法、快照、战报、PVE/PVP
  - **v1.1**：六步竖切 S1～S6；§12 战斗全流程重点板块
  - **v1.2（2026-08-05）**：**DND+自走棋**棋盘；x 三区；默认 6 格；阵法四象
  - **v1.4（2026-08-05）**：障碍/深渊探路揭晓；每回合 2 AP；[`设计文档/M3自动移动攻击算法设计.md`](./设计文档/M3自动移动攻击算法设计.md)；决策 **D9**
  ...
  - **算法文档 v2.3 + 成型设计 v1.4.4（2026-08-05）**：**战报零保留**——响应即战报，浏览器 `sessionStorage` 自存（登出/关闭销毁）；服务器不落任何战斗日志（`battle_reports` 表与 `GET /battle/reports*` 端点移除，v2.2 的 summary+setup+seed 落库方案废止；守方过程可见性登记 **M3-D04**）；**体力系统 D10**（成型 §12.9）——`stamina.yaml` + 角色 `stamina`/`stamina_updated_at` 惰性恢复 + P0 开战扣减 + `40049`，日总量天然封顶并兼作权威限流（容量口径见算法文档 §14.0：日志写入 0 字节，DB 只剩结算小事务）；S4 重定义为「战报回放（会话内）+ 体力」；前端目录设计同步（移除 reports 端点、`sessionReports`+`StaminaBar`、留存策略明示文案）；`设计文档/后续待完成.md` 新增 **M3-D04 / M3-D05**
  - [`设计文档/M3前端目录与路由设计.md`](./设计文档/M3前端目录与路由设计.md)：`/formation` `/battle`；进度对齐六步与坐标/演算
  - `设计文档/后续待完成.md`：D10～D13、D31 设计中；**M3-D01** / **M3-D02**（完整 LOS 延后）

### Changed

- **设计文档同步 OOP（2026-08-04）**：M0/M1/M2 设计与前端目录文档、`设计文档/开发计划.md` §0.6、`设计文档/后续待完成.md`（M2-D00 已消化 / M2-D01 仍待做）、README 后端架构对照已更新

- **注释中文化（2026-08-04）**：`backend/app/core/deps.py` 各 `get_*_service` 文档字符串改为中文

- **延后项登记（2026-08-04）**：`设计文档/后续待完成.md` 新增 **M2-D01**（玩法配置源抽象：现行 YAML 底表；运营化后再考虑 `ConfigSource` + 可选 DB 覆盖；勿过早全量进库）
...
- **结算模型文档正名（2026-08-04）**：与代码对齐——现行为「客户端预测展示 + 服务端惰性权威入账 + tick 对齐 `/idle/sync`」，废弃「固定 5s 轮询惰性结算」表述。
  - 更新：`设计文档/M1核心循环设计.md` §4 / `设计文档/M1前端目录与路由设计.md` §4.3 / `设计文档/M2成长深度设计.md` §4 / `设计文档/M2前端目录与路由设计.md` §4.4 / README / `设计文档/后续待完成.md`
  - `idle_service` 模块注释同步；算法本身未改

### Added

- **M2 设计文档两件套（2026-08-04）**：先定契约与大厅落点，后实现前后端。
  - `设计文档/M2成长深度设计.md` / `设计文档/M2前端目录与路由设计.md`
  - `设计文档/后续待完成.md`：M1-D01～D05、M1-D32 自设计中 → **已消化**（实现完成后）

### Changed

- **M1 里程碑收口（2026-08-04）**：
  - README / 前端设计 §9.3：进度改为「M1 已验收收口」；下一步指向 M2 + `设计文档/后续待完成.md`
...
- **修炼实时机制（无手动同步，2026-08-03）**：
  ...
  - 真 WS/SSE 推送登记为 `设计文档/后续待完成.md` M1-D33 → M6
...
- **M1 核心循环前后端实现（2026-08-03）**：挂机 → 突破 → 教学 PVE 可本地联调。
  ...
  - 延后项仍见 `设计文档/后续待完成.md`（未提前实现棋盘/三向/离线帽等）

- **M1 设计文档三件套（2026-08-03）**：尚未写玩法代码，先定契约与延后边界。
  - `设计文档/M1核心循环设计.md`：配置驱动境界；惰性挂机结算；同步突破；数值对撞 PVE；API `/idle` `/breakthrough` `/battle` `/gm`
  - `设计文档/M1前端目录与路由设计.md`：不新增路由；大厅替换挂机占位并加突破/战斗面板
  - `设计文档/后续待完成.md`：跨里程碑延后项活文档（棋盘→M3、三向/离线帽→M2 等）；**每次开新设计前必读**
...
- `设计文档/M0前端目录与路由设计.md` §7：开发进度与步骤 3 完成清单、下一步创角建议。
...
- `设计文档/M0前端目录与路由设计.md`：标注针对 **M0**、设计日期 **2026-07-27**；约定 `src` 目录职责与 `/login` 等路由对应文件。
...
- Task 7 文档收尾（2026-07-28）：`README.md` 增补 DEBUG 跳过说明、核验 API 表、超级密码注意事项、第三方 Provider 推荐；`设计文档/M0前端目录与路由设计.md` §7 步骤 3.5 标 **已完成**；设计规格状态行 → 已实现。
