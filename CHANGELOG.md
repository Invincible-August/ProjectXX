...
- **稀有度显示统一（§0.0.3）**：共用 `rarityDisplay.ts` + `RarityBadge`；列表芯片靠色、详情/选卡用中文徽章；已落到功法词条、灵宠列表/图鉴/详情、大道开道与道池图鉴；`开发计划.md` 写入强制规则。（2026-09-01）
...
- **测试服运维文档**：根目录新增 [`测试更新指令.md`](./测试更新指令.md)（conda + screen + Nginx `:8080` / 后端 `:8100`；完整更新与按范围精简步骤）（2026-08-13）
...
- **开发计划 v3.1 · 延后项镜像入里程碑**：[`后续待完成.md`](./后续待完成.md) 全部「待做」项归入 [`开发计划.md`](./开发计划.md) **§1.2 计划队列**及 M3～M13 / ADM 各节「待做队列」；§20 下一主线改为 **M8（ATTR-D02）**；登记册 §0.1 同步（2026-08-12）

- **ATTR-D01 落地（schema 占位竖切）**：`combat_attrs.yaml` + ADM 域；`build_combat_attrs` / `LifeAttrBlock`；`CharacterPublic.combat`+`life` + `GET /characters/me/combat`；大厅 `CharacterPanel` 分栏与来源拆解；`atk`=`phys_atk` 别名兼容；单测 `test_combat_attrs`；设计见 [`ATTR战斗属性占位设计.md`](./ATTR战斗属性占位设计.md) **v1.2**（2026-08-12）

- **ATTR 占位设计 v1.1（仅文档）**：[`ATTR战斗属性占位设计.md`](./ATTR战斗属性占位设计.md)——实体扩至玩家/化身/灵宠/傀儡/NPC/怪/Boss/宗门大阵；战斗键拆物法攻防+命中闪避+七维抗性+力敏智悟根；新增 `LifeAttrBlock`（体力/心魔天劫抗/吐纳/耐力/灵巧/精密/心性）；`atk`/`defense` 别名锁定；开发计划 §0.6.2 / README / 后续待完成同步（2026-08-12）
