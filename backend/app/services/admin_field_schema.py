...
FRIENDS_SCHEMA = DomainEditSchema(
    domain_id="friends",
    title_zh="道友",
    description_zh="道友上限与申请过期（M7 L2）。",
    edit_modes=("json",),
    fields=(
...
MENTOR_SCHEMA = DomainEditSchema(
    ...
    fields=(
        ...
        _f("pass_cultivation", "传功规则", "费用/加成/日帽", "json"),
...
DUAL_CULTIVATION_SCHEMA = DomainEditSchema(
    ...
    title_zh="双修",
    description_zh="功法、掷骰档、四榜（M7 L7）。",
    edit_modes=("json",),
    fields=(
        _f("invite_expire_sec", "邀约过期秒", "", "int"),
        _f("max_rerolls", "可重掷次数", "不含首次", "int"),
        _f("spirit_stone_cost", "开局灵石", "邀请方支付", "int"),
        _f("rank_min_scores", "上榜门槛", "number_one/zero", "json"),
        _f("rank_labels", "四榜中文名", "", "json"),
        _f("dice_tiers", "掷骰档表", "出目→倍率/时长", "json"),
        _f("techniques", "双修功法表", "id→定义", "json"),
    ),
)
