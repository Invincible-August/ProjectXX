...
> **战斗完善专栏**（抗性/异常/诅咒分类口子）：[`战斗系统完善设计.md`](./战斗系统完善设计.md) **v0.1**；公式未开，勿在本文假装已结算  
> 版本：v1.5 · 2026-08-05  
...
| 项 | 对齐 |
| --- | --- |
| 领域对象 | `PlayerCharacter` / 未来 Avatar·Pet·Puppet·Monster → 棋盘单位；Formation / Environment 层 |
| 配置 | 经 ContentStore（formations / monsters / stamina…）；测试可 `yaml_authority` |
| 战斗 | 养成门面 `on_battle_enter` → BattleUnitSeed；`simulate_battle` **纯度不变** |
| 增量 | 新特效走 Ability grants；禁止按单位 id 写死引擎分支 |
