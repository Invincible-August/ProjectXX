...
偏挂机的修仙题材 Web 游戏（Vue 3 + Python FastAPI）。玩法见 `设计文档/project修仙.md`，排期见 `设计文档/开发计划.md`。

设计稿在 [`设计文档/`](./设计文档/)；Agent 规则与 Skills 在 [`约束/`](./约束/)。用 Cursor 打开本仓库时如何生效，见 [`Cursor约束配置说明.md`](./Cursor约束配置说明.md)。

## 文档

| 文档 | 说明 |
| --- | --- |
| [Cursor约束配置说明.md](./Cursor约束配置说明.md) | **Cursor 规则 / Skills / AGENTS.md 如何配置与确认** |
| [AGENTS.md](./AGENTS.md) | Agent 总宪章（打开仓库自动进入上下文） |
| [约束/](./约束/) | 规则与 Skills 正文；与 `.cursor/rules`、`.cursor/skills` 同步 |
| [设计文档/](./设计文档/) | GDD、开发计划、各系统设计 |
| [project修仙.md](./设计文档/project修仙.md) | 游戏设计文档（GDD）**v4.2**（§11 灵宠自走棋硬顶 1；§12.6 傀儡神识乘区） |
| [后台管理系统开发计划.md](./设计文档/后台管理系统开发计划.md) | **后台细则 v2.0**（玩家管理整改轨 · 配置契约保留 · `/management`）；与主计划 **§1.1 ADM-P\*** 同步 |
| [开发计划.md](./设计文档/开发计划.md) | 分里程碑开发计划（**v3.19**：§10 **M8** 已收 · **§10.1 M8+** 类型与实例整改设计中 · 拍板前不开 **M9**） |
| [后续待完成.md](./设计文档/后续待完成.md) | **开新设计前必读**：延后项登记册；**M8 R0～R6 已落地**；**§1.14 INST-D00** 待拍板；延后 **M8-D01～D06** |
| [四象禁制与环境天气载荷设计.md](./设计文档/四象禁制与环境天气载荷设计.md) | **M3-D07**：禁制子类 / 环境·天气战斗载荷 / 战报中文；**v1.1.1 已落地**（载荷与 LOS 已解耦） |
| [异步真读条突破设计.md](./设计文档/异步真读条突破设计.md) | **M5-D05 / M1-D20**：可选闭关读条；**v1.2 默认关闭**——点击同步出结果（2026-08-10） |
| [灵宠系统设计.md](./设计文档/灵宠系统设计.md) | **灵宠细则**（热插拔注册表/品阶/词条/技能/被动/喂养/野外捕获/回合制对战/灵兽宗改类型/蛋孵）；**v2.2.1**（共享神识池含傀儡；2026-08-18） |
| [化身系统设计.md](./设计文档/化身系统设计.md) | **化身细则**（**v1.3**：修士可突破、禁道主/轮回境、取消独战；**D01～D10 已落地**） |
| [阵法部署与自研设计.md](./设计文档/阵法部署与自研设计.md) | **阵法部署契约**（`deploy` 四模式 / 地形 brush 定稿 / `force_shifts` / ADM+自研同 schema）；**M3-D08 Phase A 已落地**；**Phase B 自研设计器已落地（M8 R3）**；**v1.3**（2026-08-17） |
| [符箓叠层与异常状态设计.md](./设计文档/符箓叠层与异常状态设计.md) | 符箓同种类不叠加 / 攻击触发 / 异常状态回合序（**v1.1**；引擎挂 M3-D03 / M8-D04） |
| [战斗系统完善设计.md](./设计文档/战斗系统完善设计.md) | **战斗专栏 v0.1**：抗性三族（元素/异常/暗）与异常·诅咒分类口子；公式未开 |
| [功法系统设计.md](./设计文档/功法系统设计.md) | **功法专栏 v0.2**：主功法/技法双角色、统一悬停 format、紧凑选择栏；学习条件口子、五源 |
| [神通系统设计.md](./设计文档/神通系统设计.md) | **神通专栏 v0.2.1**：神通池点槽选装、无主副、格数=修为+品阶；i 文案与紧凑格 |
| [M0工程骨架设计.md](./设计文档/M0工程骨架设计.md) | **M0** 框架与接口字段详细设计 |
| [M0前端目录与路由设计.md](./设计文档/M0前端目录与路由设计.md) | **M0** 前端 `src` 目录职责与路由对照（2026-07-27） |
| [M1核心循环设计.md](./设计文档/M1核心循环设计.md) | **M1** 挂机 / 突破 / 极简战斗详细设计（2026-08-03） |
| [M1前端目录与路由设计.md](./设计文档/M1前端目录与路由设计.md) | **M1** 大厅内面板落点（无新路由）（2026-08-03） |
| [M2成长深度设计.md](./设计文档/M2成长深度设计.md) | **M2** 三向挂机 / 离线帽 / 分配 / 体质 / 品阶详细设计（2026-08-04） |
| [挂机速率与加成设计.md](./设计文档/挂机速率与加成设计.md) | **挂机速率**：境界基础表 × 内外部加成；`idle.yaml` / `idle_env` 拆解（2026-08-06） |
| [骰子系统设计.md](./设计文档/骰子系统设计.md) | **修为检定骰**：YAML 查表上下限 + 动态修正；突破/战斗/双修统一框架（2026-08-06） |
| [M2前端目录与路由设计.md](./设计文档/M2前端目录与路由设计.md) | **M2** 大厅面板与对话框落点（仍无新玩法路由）（2026-08-04） |
| [M3战斗成型设计.md](./设计文档/M3战斗成型设计.md) | **M3** 现行已落地设计（S1～S6）；**v1.5**；延后见 `后续待完成` M3-D01～D10（2026-08-05） |
| [M3自动移动攻击算法设计.md](./设计文档/M3自动移动攻击算法设计.md) | **M3** 默认 AI；**v2.5**：友军软墙绕行 + 让路；现行 vs 延后分界；Tier B/最小 LOS/零保留 |
| [M3前端目录与路由设计.md](./设计文档/M3前端目录与路由设计.md) | **M3** 新增 `/formation` `/battle`；大厅变枢纽（2026-08-04） |
| [M4双线程成长设计.md](./设计文档/M4双线程成长设计.md) | **M4** 化身双挂机 / 工坊配方 / 灵宠雏形 / 神识骨架；N4 收窄出口（2026-08-05 / 07） |
| [M4前端目录与路由设计.md](./设计文档/M4前端目录与路由设计.md) | **M4** 新增 `/avatar` `/workshop` `/pets`；大厅双线程入口（2026-08-05） |
| [M5环境与轮回外环设计.md](./设计文档/M5环境与轮回外环设计.md) | **M5** 六时/天气/雷劫（**仅跨境**+准备格+劫云）/待引渡与轮回强化；竖切 E1～E6；**v1.5**（2026-08-10 · 切段算力 + 中文出口） |
| [M5前端目录与路由设计.md](./设计文档/M5前端目录与路由设计.md) | **M5** `/tribulation` `/reincarnation` + 时辰天气顶栏；新生商店/轮回袋；**v1.5**（2026-08-10） |
| [M6大道与道主设计.md](./设计文档/M6大道与道主设计.md) | **M6** 开道/道池/道值运用、WS 基建、道主空位自动就任、**道主之争赛会定案**（§5 / M6-D06）；竖切 **W1～W6**；**v1.1**（2026-08-10） |
| [M6前端目录与路由设计.md](./设计文档/M6前端目录与路由设计.md) | **M6** `/dao` `/dao-lord` + `src/ws`；赛会 mode 预告；**v1.1**（2026-08-10） |
| [M7宗门社交与经济设计.md](./设计文档/M7宗门社交与经济设计.md) | **M7** 宗门/交易/邮件/多频道聊天/传承/师徒/双修/商业化壳；竖切 **L1～L8**；**v1.0**（2026-08-11） |
| [M7前端目录与路由设计.md](./设计文档/M7前端目录与路由设计.md) | **M7** `/sect` `/market` `/social` `/friends` `/party` `/dual-cultivation` `/shop` + ChatDock；**v1.0**（2026-08-11） |
| [道具与装备实体设计.md](./设计文档/道具与装备实体设计.md) | **M8 R0**：17 区穿戴栏、化身开关、符箓编成、傀儡最多 3 只、双手双指针、丹药方案 A、福宝不可逆耐久、四页背包、穿戴悬停 format、工坊成品单一属性 `inspect`；**v0.8.11**（2026-08-19） |
| [M8自研与内容管线设计.md](./设计文档/M8自研与内容管线设计.md) | **M8** 自研功法/阵法/符箓 + ATTR-D02 装备喂属性 + 扩表校验；竖切 **R0～R6 已落地**；洞府 `/cave` + 研究室 `/cave/lab`；**v1.1.18**（2026-08-20） |
| [M8前端目录与路由设计.md](./设计文档/M8前端目录与路由设计.md) | **M8** `/cave` 洞府、`/cave/workshop` 工坊、`/cave/lab` 研究室；角色页本尊/化身同页切换 + **17 区示意剪影穿戴栏**；悟道挂本尊属性卡头；`PlayNav` 含账号同一组按钮；工坊四分支（无阵法）+ 两列配方/队列；布阵只摆子；**v1.1.36**（2026-08-25） |
| [ATTR战斗属性占位设计.md](./设计文档/ATTR战斗属性占位设计.md) | **ATTR-D01 已落地**：统一战斗+生活属性 schema；**v1.4.2** 根基含悟性/耐力/神通（2026-08-18） |
| [统一实体类层次与重构方案.md](./设计文档/统一实体类层次与重构方案.md) | **OOP/ABC v1.0.6**：`CultivatorCharacter` 修士；化身叶 + `DivineSenseConsumer`；灵宠/傀儡仍 `SenseMinionCharacter` |
| [双轨整改与配置兼容方案.md](./设计文档/双轨整改与配置兼容方案.md) | **双轨 v1.0**：存量整改 / 增量规范；ContentStore（测试 YAML / 正式可 DB）（2026-08-14） |
| [玩家在线状态设计.md](./设计文档/玩家在线状态设计.md) | **Presence**：WS 鉴权在线 / grace / 组队·面交·助战门闸；**v1.0**（2026-08-12） |
...
| [测试更新指令.md](./设计文档/测试更新指令.md) | **测试服更新/重启**：conda + screen + Nginx（8080/8100）；完整更新与按改动范围精简步骤 |
| [CHANGELOG.md](./CHANGELOG.md) | 变更记录 |

## 当前进度

- **事件日志持久化（2026-09-01）**：按角色 localStorage 保存约 24h；切页/刷新保留，登出清空。
- **功法基础加成次数（2026-09-01）**：显示剩余可升级；上限=功法阶每阶次数累加 + 创建时法修/体修境界每阶额外（YAML/后台 research）。
- **功法词条栏与突破成长（2026-09-01）**：创建栏数=人物境界；突破补空位并可推演；已有词条阶强化，新位无强化。
...
- **功法可修炼层数 / 大圆满（2026-09-01）**：可修炼功法经资源分配升至 10 层再入大圆满；五层/圆满奖励库；自创定稿前须选定两档奖励；卡片培养仍走研究室。
- **功法自研初始阶与废除（2026-08-31）**：定稿从锻体起逐步突破；可废除（须卸装，只删作者本门，流通副本保留）。
- **功法自研 UI（2026-08-31）**：研究室「心法雏形」双栏；定稿名唯一；培养态重登不丢；突破显示所需升级点；【测】无限正式卡由运营后台角色管理发放；词条品阶浅底深字+结构化悬停（2026-09-01）。
- **稀有度显示规范（2026-09-01）**：凡 rarity 展示统一走 `frontend/src/utils/rarityDisplay.ts` 与 `RarityBadge`（列表靠色、详情徽章）；见 `设计文档/开发计划.md` **§0.0.3**。**物品**（工坊造物/装备）品阶与词条同为粗糙～太古七档；突破品阶/灵宠养成品阶另表。
- **物品格展示（2026-09-01）**：有图标用图、无图标用中文名；预留 UI 接口、不强制短名；见 `设计文档/开发计划.md` **§0.0.4**。图标键放 `frontend/src/assets/items/{icon}.svg|png|webp`，组件 `ItemSlotVisual`。
...
- **工坊配方收束（2026-08-19）**：两列配方/队列；材料=材料+灵石，体力另栏；悬停单一属性（金木水火土风雷暗），使用条件仅境界
- **资源分配可超额（2026-08-19）**：投入可超过本档门槛；突破/淬体只扣所需；超额弹窗确认
- **M8+ 类型与实例（2026-08-19 · 登记）**：下一主线；类型白板 / 实例最终属性；清单 [`设计文档/后续待完成.md`](./设计文档/后续待完成.md) **§1.14**；拍板前不开 M9
- **阵法页改版（2026-08-19）**：顶栏「阵法」；预设下拉最多 5 套可命名；棋子四栏（角色/灵宠/傀儡/化身）按装备计数；棋盘不显示坐标数字；未编成傀儡（含试炼木傀）不再可上场（前端同步过滤 `puppet_N`）
- **战斗专栏口子（2026-08-18）**：[`设计文档/战斗系统完善设计.md`](./设计文档/战斗系统完善设计.md) **v0.1**；[`设计文档/功法系统设计.md`](./设计文档/功法系统设计.md) **v0.1**；`PlayNav` 含账号同一组按钮；抗性并入战斗属性；功法方格装备 + 灵根展示
- **前端生产构建**：`cd frontend && npm ci && npm run build`（`vue-tsc -b && vite build`）在 2026-08-12 已通过类型检查修复后可完整产出 `dist/`
- **M8 出口打磨已收（2026-08-17）**：渡劫/待引渡页级只读；阵法脏离开；切 mode 清 session；`smoke_m8` 含 40211 门禁断言
- **M8 R6 已落地（2026-08-17 · 校验器与总装）**：`python -m app.config_source.validate_content`；大厅「工坊」「自研」「继续草案」；`scripts/smoke_m8.py`；单测 `test_content_validator`
- **M8 R5 已落地（2026-08-17 · 宗门图纸）**：`manual`+`manual_kind`；代工真扣材料；兑换入包；捐赠 `40209`；藏宝阁拒图纸；单测 `test_sect_blueprint_item_type`
- **M8 R4 已落地（2026-08-17 · 自研符箓）**：`/research?mode=talisman` 白名单定稿 `custom:talisman:…`；工坊 `?branch=talisman` 画符+预载；开战战报 `item_trigger` 中文「符箓：××」；单测 `test_research_talisman_whitelist`
- **M8 R3 已落地（2026-08-17 · 自研阵法）**：`/research?mode=formation` 设计器 + 定稿 `custom:formation:…`；布阵 picker 自研分组；防守快照内联蓝图；单测 `test_research_formation_blueprint`
- **M8 R0 已落地（2026-08-18）**：[`设计文档/道具与装备实体设计.md`](./设计文档/道具与装备实体设计.md) **v0.8.6**——工厂/四法则/四页/丹药白名单；**17 区穿戴 + 傀儡编成最多 3 只 + 双手双指针**；角色页示意剪影槽名（主手/项链/符宝）
- **M8 R2 已落地（2026-08-17 · 自研功法）**：`/research?mode=technique` + `/research/sessions*`；定稿私有功法 `custom:technique:…`；角色页功法来源列；审核池占位 `40210`
- **M8 R1 已落地（2026-08-17 · ATTR-D02）**：`/equipment/*` + 角色页 `EquipmentSlotsPanel`（槽表 **17 区** + 编成板）；`build_combat_attrs` / 挂机 / 骰子装备通道
- **M8 设计（2026-08-17 · 核心 v1.0.3 / 前端 v1.1.1）**：[`设计文档/M8自研与内容管线设计.md`](./设计文档/M8自研与内容管线设计.md) / [`设计文档/M8前端目录与路由设计.md`](./设计文档/M8前端目录与路由设计.md)——竖切 **R1～R6**；前端含布局/样式文字方案
- **ATTR-D01 已落地（2026-08-18 · v1.4.2）**：[`设计文档/ATTR战斗属性占位设计.md`](./设计文档/ATTR战斗属性占位设计.md)——抗性并入战斗属性；简介轮回点/体力；根基含悟性/耐力/神通；装备喂入 → ATTR-D02（**M8 R1**）；满曲线 → M13
- **ARCH 底层重构已收口 S1（2026-08-14 · v1.0.2）**：[`设计文档/统一实体类层次与重构方案.md`](./设计文档/统一实体类层次与重构方案.md) / [`设计文档/双轨整改与配置兼容方案.md`](./设计文档/双轨整改与配置兼容方案.md)——`app/game` + ContentStore + 傀儡 Actor + 道主 privilege + shim 收敛；**下一主线 M8 / ATTR-D02**
- **玩家在线状态 Presence 已落地（2026-08-12）**：[`设计文档/玩家在线状态设计.md`](./设计文档/玩家在线状态设计.md) **v1.0**——`PresenceService` + Hub 索引/grace；组队/面交/助战/道友/赛会共用；`presence.changed` 推送；多 worker → **PRESENCE-R01**
- **M7 已收口（2026-08-13）**：L1～L8 + V+ + 打磨包（真正坊市、大厅导航、社交交易/双修/师徒等）；`scripts/smoke_m7.py`；设计见 [`设计文档/M7宗门社交与经济设计.md`](./设计文档/M7宗门社交与经济设计.md)；延后见 [`设计文档/后续待完成.md`](./设计文档/后续待完成.md) **CHAT-D03** / **M7-D***
...
- **ADM 后台 v2.0（2026-08-14）**：同端口 `/management`；整改首轨 **玩家管理 → 账号管理**（含打赏/仙缘/广告流水）；细则 [`设计文档/后台管理系统开发计划.md`](./设计文档/后台管理系统开发计划.md) **v2.0**；配置双写契约仍见 §0.0.1
...
- **灵宠愿景 + N4/N5/PET-D01～D06/M4-D04c 落地（2026-08-07）**：[`设计文档/灵宠系统设计.md`](./设计文档/灵宠系统设计.md) **v2.1**；真地图 region → M9
- **化身独立穿戴 + 大厅修炼日志（2026-08-19）**：装备/功法/神通 `actor` 分槽；破除转修为、装备回包、再凝练金丹起；大厅 10 周天结算日志
- **化身定案 + 深化落地（2026-08-07）**：取消多化身；[`设计文档/化身系统设计.md`](./设计文档/化身系统设计.md) **v1.2 落地 / v1.3 设定冻结**；**AVATAR-D01～D06** 已落地；**D07～D10** 待做；**OOP/减负**：CapabilityIndex 预计算、体力脏写、`get_summary` 轻量摘要
- **异步真读条突破已落地后默认关闭（2026-08-10）**：[`设计文档/异步真读条突破设计.md`](./设计文档/异步真读条突破设计.md) **v1.2**——玩家点击突破同步出结果；`async_channel` 可运营再开；API `/breakthrough/channel/*` 保留
- **四象加深 M3-D07 Phase A 落地（2026-08-10）**：[`设计文档/四象禁制与环境天气载荷设计.md`](./设计文档/四象禁制与环境天气载荷设计.md) **v1.1**——禁制三子类 LOS；迷雾/雷暴 combat 载荷；战报中文；样本金锁障·咒缄幕·万缄障
- **阵法部署 M3-D08 Phase A 落地（2026-08-10）**：[`设计文档/阵法部署与自研设计.md`](./设计文档/阵法部署与自研设计.md) **v1.1**——`deploy` 四模式 / `force_shifts` / presets `effective_deploy_cells` / 前端换阵高亮；样本 wide_front·cloud_drift·left_wing_mask·shift_gust
- **嘲讽光环 M3-D06 Phase A 落地（2026-08-10）**：[`设计文档/嘲讽光环设计.md`](./设计文档/嘲讽光环设计.md) **v1.1**——静态 `taunt_auras` / 进入触发 / 决策短路 / 死亡解除 / 样本 `taunt_guardian` / ADM 域
- **延后/并行项**：总表见 [`设计文档/开发计划.md`](./设计文档/开发计划.md) **§1.2**；细则钩子见 [`设计文档/后续待完成.md`](./设计文档/后续待完成.md)（M3 §1.5 · M4 §1.6 · M5 §1.7 · M6 §1.11 · M7 §1.12 · ATTR/CHAT/IDLE/DICE）
- **扩展层已排期（开发计划 v3.17）**：~~M6~~ / ~~M7~~ → **M8 R0～R6 已落地** → M9～M13；新功能须遵循 **§0.7 显性设计**、**§0.0 后台适配**、**§0.0.2 中文信条** 与 **§0.6.2 属性路线**
- **M7 L1～L8 已收口**（含 V+ 与打磨包）；后端测试含 M0～M8 + ADM；冒烟 `smoke_m3` / `smoke_m4` / `smoke_m5` / `smoke_m6` / `smoke_m7` / `smoke_m8` / `smoke_adm`
- **后端 OOP 分层**：见下方「后端架构」
...
game/           # OOP 实体层 Character/Item/Ability/ContentStore（ARCH v1.0）
core/           # Settings / JWT（玩家+后台）/ time_utils / deps 工厂
db/             # ORM（含 admin_users / config_*）+ bootstrap
schemas/        # HTTP DTO（Pydantic）
```

> **OOP 重构（v1.0.6）**：`app/game` 实体层——`Character` 含 hp/mp；**修士** `CultivatorCharacter`（玩家/NPC/化身）；玩家/NPC 持神识池；化身上阵消耗神识（`DivineSenseConsumer`）；灵宠/傀儡 `SenseMinionCharacter`。细则 [`设计文档/统一实体类层次与重构方案.md`](./设计文档/统一实体类层次与重构方案.md)。测试可设 `CONTENT_STORE_MODE=yaml_authority` 只改 YAML。
...
| 配置热更 | `.env` `CONTENT_STORE_MODE` → `ContentStore` → `GameConfigBundle`（`yaml_authority` 只 YAML；默认 YAML∪DB 发布） |
| **常量** | **`app/constants/`**（开发计划 **§0.6.3 法律级**）：协议枚举/映射/魔数统一管理，须中文注释；禁止业务文件新增裸魔数 |
...
**详情对照**：各 `M*` 设计 §分层 · [`设计文档/开发计划.md`](./设计文档/开发计划.md) §0.6 · [`设计文档/后台管理系统开发计划.md`](./设计文档/后台管理系统开发计划.md)
...
- **后台整改（进行中）**：侧栏 **玩家管理 → 账号管理 / 角色管理**
  ...
  - 搜索：邮箱 / 手机 / user_id；分页 10/20/50/100；登录对无效账号提示「无效用户名」
  - API：`GET/POST /admin/ops/players*` · `/admin/ops/characters*`（写操作须 publisher/admin）
  - 旧配置域 / 道主控制台 / 审计菜单暂下线（路由重定向到账号管理），后续按类目重建
...
- 修改密码：`POST http://127.0.0.1:8000/api/v1/auth/change-password`  
  ...
  若 `REGISTER_REQUIRE_EMAIL_CODE=true`：须先 `POST /verification/email/send|confirm` 换 `email_ticket` 一并提交（与注册同源）
- 账号摘要：`GET http://127.0.0.1:8000/api/v1/account/summary`（仙缘余额 / 累计打赏 / 累计观看广告次数）
- 打赏账单：`GET http://127.0.0.1:8000/api/v1/account/tips`（时间 / 金额 / 单号 / 途径）
...
| GET | `/api/v1/techniques/me` | 角色功法等级列表（含 `source` / `source_label_zh`）；`actor=main|avatar` 分槽 |
| GET/POST | `/api/v1/constitution/*` | 体质收藏 / 本源·旁支镶嵌 / 卸下 / 升品 / 融合（骨架） |
| POST | `/api/v1/gm/character/set` | 仅 development；支持 `realm_progress` / `clear_offline_pending` 等 |

### M8 装备与洞府研究室 API（需 Bearer）

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/v1/equipment/slots` | 17 区指针槽穿戴（含 `pet`）+ 傀儡编成列表；`actor=main|avatar` 分槽 |
| POST | `/api/v1/equipment/equip` | 穿戴（双手武器双写两槽） |
| POST | `/api/v1/equipment/unequip` | 卸下（双手同清两侧） |
| POST | `/api/v1/equipment/puppet-loadout/add` | 傀儡加入编成板 |
| POST | `/api/v1/equipment/puppet-loadout/remove` | 傀儡移出编成板 |
| GET | `/api/v1/equipment/catalog` | 可穿样本 |
| GET | `/api/v1/cave` | 洞府房间列表（研究室） |
| GET | `/api/v1/cave/lab/catalog` | 研究室材料/词条白名单 |
| GET | `/api/v1/cave/lab/mine` | 已定稿私有内容 |
| GET | `/api/v1/cave/lab/sessions` | 进行中草案列表（大厅「继续草案」） |
| POST | `/api/v1/cave/lab/sessions` | 开会话（`kind=technique\|formation\|talisman`） |
| GET | `/api/v1/cave/lab/sessions/{id}` | 会话预览 |
| POST | `/api/v1/cave/lab/sessions/{id}/draft` | 保存阵法蓝图或符箓效果 |
| POST | `/api/v1/cave/lab/sessions/{id}/reroll` | 重投词条（仅功法） |
| POST | `/api/v1/cave/lab/sessions/{id}/finalize` | 定稿中文名 |
| POST | `/api/v1/cave/lab/sessions/{id}/submit-review` | 审核池占位，恒 `40210` |
| POST | `/api/v1/craft/talisman/scribe` | 画符入包 |
| GET/PUT | `/api/v1/craft/talisman/preload` | 出战预载栏 |
...
### 新增一条官方样本（只改 YAML / 后台，不改玩家代码）

> 校验器：启动加载与 CI 共用。本地可跑：
>
> ```powershell
> cd backend
> .\.venv\Scripts\python.exe -m app.config_source.validate_content
> .\.venv\Scripts\python.exe -m pytest tests/test_content_validator.py -q
> ```
>
> 坏表（未知 ATTR 键、符效果缺 `label_zh` / 非法 `trigger`、体质词条发明新 effects 键）→ **拒绝启动** / 发布探针 `40050`。

**例：新增一件样本装备**（须同时在 `inventory.yaml` 与 `equipment.yaml` 登记同 id；`stats` 键 ∈ `combat_attrs.yaml`）：
...
**例：新增一条符效果白名单**（自研只能引用已发布 id）：

```yaml
# talisman_effects.yaml
first_hit_ward:          # 已有样本
  label_zh: 护体残符
  help_zh: …
  trigger: first_hit     # 仅白名单 trigger
```

**例：新增自研词条**（`research.yaml` → `affixes`；`stats` 键须已登记 ATTR）：
...
| GET | `/api/v1/formation/presets` | 预设五槽 + 已解锁阵法 + 可上阵棋子（Bench）+ 上阵上限 + `assist_guest` |
| PUT | `/api/v1/formation/presets/{slot}` | 保存预设：`{name, role, formation_id, units, assist_anchor}`（slot 0～4；`role` 仅开战/快照回退；`assist_anchor` 为助战虚位）；非法占位 `40041/40042/40043`；有助战会话无锚点开战 `40093` |
...
| GET | `/api/v1/avatar/features` | 功能解锁看板 + 下一档预告 + **`condense` 权威闸**（`can_condense`/`realm_ok`/`stones_ok`/`cultivation_ok`/功法·媒介候选；与 POST 同源） |
| POST | `/api/v1/avatar/condense` | 凝练化身（金丹**及以上**门槛 `40050`；已有化身 `40051`；灵石不足 `40000`；灵力不足 `40053`；功法 `40054`；媒介不足 `40055`；Body `technique_id` + `medium_item_id`） |
| POST | `/api/v1/avatar/dismiss` | 破除已凝练化身：修为池全额转入本体、淬体丢弃、装备回背包；不退凝练材料；工坊占用中 `40056` |
| POST | `/api/v1/avatar/idle` | Body `{"direction":"spirit"\|"body"\|"crafting"\|"sect_mining"\|"none"}`；采矿需入宗且计入矿脉名额；未解锁方向 `40090`。回包 `{avatar, character, idle_gains}`，化身锚点与本体独立 |
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/dao/catalog` `/dao/me` `/dao/pool` | 图鉴 / 本命资源 / 道池（`actor=main\|avatar`；须该主体真仙） |
| POST | `/dao/open/roll` `/dao/open/choose` | 开道三选一（body `actor`） |
...
角色摘要/角色页展示体力（`battle_stamina.left/cap`）。
...
**环境锁定**：开战、工坊开工在**点击确认那一刻**写入时辰/天气快照。工坊页不再展示页顶天气占位行；任务/战斗进行中世界天气再滚动，不影响本场已锁环境。

角色面板 `CharacterPublic.idle_env`：spirit/body/crafting 有效挂机速率 = **大境界基础表** × 加成通道（体质钩子等）× 时辰×天气×灵根/功法 `env_tags`；`breakdown` 可拆解（含 `realm_base` / `constitution` 等）。细则见 [`设计文档/挂机速率与加成设计.md`](./设计文档/挂机速率与加成设计.md)。大厅修炼区默认只显示「基础→有效」与总乘区，乘区拆解与时辰/天气说明悬停旁侧 **i** 查看。
...
检定/先攻/伤害骰一律按 [`设计文档/骰子系统设计.md`](./设计文档/骰子系统设计.md)：大境界+小境界查表得默认上下限，再叠功法/体修/气运等修正；`board.dice_sides` 仅作兼容回落。
...
3. 进入 `/hall`：**左侧**角色摘要（链到 `/character`）+ 修炼 + 突破 + 战斗 +（DEV）调参；**右侧**事件日志。角色页 `/character`：左属性（详参折叠）、右体质装备与功法（功法卡「去研究室」进 `/cave/lab?mode=technique`）。
...
| M8～M13 | 自研 / 世界空间 / 叙事 / 世界事件 / AI / 内容数值 | **已排入** [`设计文档/开发计划.md`](./设计文档/开发计划.md) **v3.2**（§1.2 延后队列 · 下一主线 M8） |

进度细节见 [M5前端目录与路由设计.md](./设计文档/M5前端目录与路由设计.md) §9、[M4前端目录与路由设计.md](./设计文档/M4前端目录与路由设计.md) §9、[M3前端目录与路由设计.md](./设计文档/M3前端目录与路由设计.md) §9。
