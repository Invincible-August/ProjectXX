...
| 项 | 对齐 |
| --- | --- |
| 领域对象 | 鉴权/账号壳；创角后包装为 `PlayerCharacter`（ORM `Character` 改名延后 R6） |
| 配置 | 经 ContentStore；密钥仍走 `.env`；测试可 `CONTENT_STORE_MODE=yaml_authority` |
| 战斗 | M0 无战斗；预留字段不进引擎；开战契约见 M3 + ARCH |
...
*2026-08-14 补记：新增「ARCH 对齐」节；玩法与 HTTP 契约不变，见统一实体类层次与双轨整改方案。*
